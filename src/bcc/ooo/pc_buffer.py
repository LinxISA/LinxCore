...
def build_janus_bcc_ooo_pc_buffer(m: Circuit, *, depth: int = 64, idx_w: int = 6) -> None:
    ...
    for i in range(depth):
        ...
        lookup_match_pcb = valid_pcb[i].out() & is_bstart_pcb[i].out() & pcs_pcb[i].out().__eq__(lookup_pc_pcb)
        lookup_known_match_pcb = valid_pcb[i].out() & pcs_pcb[i].out().__eq__(lookup_pc_pcb)
