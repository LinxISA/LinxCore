...
def build_backend_exec_pipe(
    ...
    *,
    ...
    rst = m.reset("rst")
    c = m.const
    issue_meta_w = 1 + 64 + 64 + rob_w + ptag_w + 1 + 1 + 1 + 12 + 4 + 64
    issue_data_w = 64 * 5
    issue_bundle_w = issue_meta_w + issue_data_w
    w2_meta_w = 1 + rob_w + ptag_w + 1 + 1 + 1 + 4 + 64
    w2_data_w = 64 * 5

    flush_i = m.input("flush_i", width=1)
    flush_bid_i = m.input("flush_bid_i", width=64)
    ...
    for slot in range(issue_w):
        ...
        issue_fields = {
            ...
            "block_bid": slot_meta.slice(lsb=148 + rob_w + ptag_w, width=64),
        ...
        if slot == cmd_slot:
            issue_fields["cmd_block_bid"] = issue_fields["block_bid"]
