...
def classify_dispatch_target(op, op_is):
    ...
    to_alu = (~to_lsu) & (~to_d2) & (~to_bru) & (~is_macro)
    return to_alu, to_bru, to_lsu, to_d2
...
def allocate_iq_slots(*, m, p, consts, disp_valids, disp_to_alu, disp_to_bru, disp_to_lsu, iq_alu, iq_bru, iq_lsu):
...
def build_dispatch_stage(
    ...
    *,
    ...
    for slot in range(dispatch_w):
        ...
        to_alu, to_bru, to_lsu, to_d2 = classify_dispatch_target(op, op_is)
