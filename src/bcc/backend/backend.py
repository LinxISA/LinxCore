...
from .issue import build_iq_update_stage, build_issue_stage
...
def build_bcc_ooo(m: Circuit, *, mem_bytes: int, params: OooParams | None = None) -> BccOooExports:
    ...
    for slot in range(p.commit_w):
        ...
        is_start_marker = is_bstart | is_macro
        is_boundary = rob_is_boundaries[slot] | is_macro
        ...
        commit_is_bstops.append(fire & is_bstop)
        ...
        br_pred_take_live = (fire & is_boundary & br_take).select(consts.zero1, br_pred_take_live)

        enter_new_block = fire & is_bstart & (~br_take)
        ...
        macro_enter = fire & is_macro & (~br_take)
    ...
    # Keep compatibility with the current Linx toolchain/QEMU ABI behavior:
    # function templates apply a fixed outgoing-call frame addend.
    macro_callframe_size = c(64, width=64)
    ...
    bru_redirect_valid = consts.zero1
    bru_redirect_pc = consts.zero64
    bru_redirect_checkpoint_id = c(0, width=6)
    ...
    if p.bru_w > 0:
        ...
        bru_validate_en = state.br_kind.out().eq(c(BK_COND, width=3)) | state.br_kind.out().eq(c(BK_RET, width=3))
        ...
        for i in range(pcb_depth):
            hit = pcb_valid[i].out() & pcb_is_bstart[i].out() & pcb_pc[i].out().eq(bru_target)
            bru_target_is_bstart = hit.select(consts.one1, bru_target_is_bstart)

        bru_redirect_valid = bru_mismatch & bru_target_is_bstart
        bru_redirect_pc = bru_target
        bru_fault_set = bru_mismatch & (~bru_target_is_bstart)
    ...
    commit_redirect_any = commit_redirect | bru_redirect_valid
    redirect_pc_any = bru_redirect_valid.select(bru_redirect_pc, redirect_pc)
    redirect_checkpoint_id_any = bru_redirect_valid.select(bru_redirect_checkpoint_id, redirect_checkpoint_id)
    ...
    trap_pending_n = bru_fault_set.select(consts.one1, trap_pending_n)
    trap_rob_n = bru_fault_set.select(bru_fault_rob, trap_rob_n)
    trap_cause_n = bru_fault_set.select(c(TRAP_BRU_RECOVERY_NOT_BSTART, width=32), trap_cause_n)
