...
def build_janus_bcc_ifu_f3(m: Circuit, *, ibuf_depth: int = 8) -> None:
    ...
    c = m.const
    dec_bundle_f3_inst = m.instance_auto(
        build_decode_bundle_8b,
        name="dec_bundle_f3",
        module_name="LinxCoreDecodeBundle8B",
        window=f2_to_f3_stage_window_f2,
    )
    dec_bundle_f3 = decode_f4_bundle_view(m, bundle=dec_bundle_f3_inst)
    ...
    ib_bundle_f3_inst = m.instance_auto(
        build_decode_bundle_8b,
        name="ib_bundle_f3",
        module_name="LinxCoreDecodeBundle8B",
        window=ibuf_f3["out_window"],
    )
    ib_bundle_f3 = decode_f4_bundle_view(m, bundle=ib_bundle_f3_inst)
    for slot in range(4):
