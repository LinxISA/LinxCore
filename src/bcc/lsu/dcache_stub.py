...
def build_janus_bcc_lsu_dcache_stub(
    ...
    *,
    ...
    resp_ok = c(1, width=1)
    resp_err = c(0, width=err_w)

    m.output("dcache_req_ready", req_ready)
    m.output("dcache_resp_valid", resp_valid)
    m.output("dcache_resp_entry_id", resp_entry_id)
    m.output("dcache_resp_ok", resp_ok)
    m.output("dcache_resp_err_code", resp_err)
