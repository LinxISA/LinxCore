...
# NOTE: These op IDs are an internal encoding for the LinxISA bring-up cores
# (used by Janus BCC, and by the `examples/linx_cpu_pyc` reference core).
#
# Keep them stable across backends/tests (C++ + Verilog). They are *not* the
# raw ISA opcode values.
