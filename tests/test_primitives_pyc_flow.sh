...
run_case() {
  ...
  echo "[primitives] build+sim ${name}"
...
PY
  ...
  echo "[primitives] run(cpp) ${name}: ${cpp_bin}"
  (cd "${out_dir}" && "${cpp_bin}")
}

run_case "mem_mem2r1w" "${ROOT_DIR}/tests/pyc/tb_mem_mem2r1w.py"
run_case "common_uid_allocator" "${ROOT_DIR}/tests/pyc/tb_common_uid_allocator.py"

echo "primitives pyc flow passed"
