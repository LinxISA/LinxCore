...
OUT_DIR="${ROOT_DIR}/out/pyc/ooo_recovery_class_merge"
...
PYTHONPATH="$(pyc_pythonpath):${ROOT_DIR}/src" PYTHONDONTWRITEBYTECODE=1 PYCC="${PYCC}" \
  "${PYTHON_BIN}" -m pycircuit.cli build \
    "${ROOT_DIR}/tests/pyc/tb_ooo_recovery_class_merge.py" \
    --out-dir "${OUT_DIR}" \
    --target both \
    --jobs "${PYC_SIM_JOBS:-4}" \
    --logic-depth 256 \
...
if grep -R -Ei '\b(arm|aarch|cortex)\b' \
  "${ROOT_DIR}/src/bcc/ooo/recovery_class_merge.py" \
  "${ROOT_DIR}/tests/pyc/tb_ooo_recovery_class_merge.py" >/dev/null; then
  echo "error: recovery class merge must not encode ARM-specific semantics" >&2
  exit 1
fi

echo "ok: pyCircuit OOO recovery class merge flow passed"
