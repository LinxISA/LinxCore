...
CASES = (case(valid_bpcn=True), case(valid_bpcn=False))
ICALL_UIMM5 = 13
...
def main() -> int:
    ...
    assert len(rows) == 2, f"expected two ICALL rows, got {rows}"
    assert rows[0]["fire"] == 1
    assert rows[0]["tgt"] == 0x900
    assert rows[0]["cond"] == 1
