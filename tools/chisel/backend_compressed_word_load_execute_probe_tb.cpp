...
constexpr std::uint64_t kCLdiLoad = 0x8001002000010003ULL;
...
void fail(const char *message) {
    std::cerr << "backend-compressed-word-load-execute-probe: " << message << '\n';
    std::exit(1);
}
...
void drive_load(VCompressedWordLoadExecuteProbe &dut, std::uint32_t opcode, std::uint64_t imm,
                std::uint64_t load_data, bool wait_blocked = false) {
    idle(dut);
    dut.io_inValid = 1;
    ...
    dut.io_loadLookupData = load_data;
...
void wait_for_completion(VCompressedWordLoadExecuteProbe &dut, const char *context) {
    ...
    std::cerr << "backend-compressed-word-load-execute-probe: " << context << '\n';
    std::exit(1);
}
...
void prove_c_lwi_first_red(VCompressedWordLoadExecuteProbe &dut) {
    reset(dut);
    drive_load(dut, kOpCLwi, kCLwiImm, kCLwiLoad);
    require(dut.io_loadLookupValid, "first-red: accepted C.LWI did not assert loadLookupValid");
    require(dut.io_loadLookupAddr == kCLwiAddr, "C.LWI load address did not scale signed simm5 by four");
    require(dut.io_loadLookupSize == 4, "C.LWI load size was not four bytes");
    require(dut.io_loadLookupReturnSignExtend, "C.LWI did not request signed word load return");
    require_identity_lookup(dut);
    wait_for_completion(dut, "C.LWI did not complete after lookup");
    require_common_completion(dut, kCLwiResult, kCLwiAddr, 4);
}

void prove_c_lwi_wait_hold_once(VCompressedWordLoadExecuteProbe &dut) {
    reset(dut);
    drive_load(dut, kOpCLwi, kCLwiImm, kCLwiLoad, true);
    require(dut.io_loadLookupValid, "blocked C.LWI lookup was not valid");
    require(dut.io_loadWaitHold, "blocked C.LWI did not assert wait hold");
    require(!dut.io_completeValid, "blocked C.LWI completed while wait-held");
    require(!dut.io_inReady, "blocked C.LWI accepted a second input while wait-held");
    require(dut.io_loadLookupAddr == kCLwiAddr, "blocked C.LWI lookup address was not stable");
    require_identity_lookup(dut);

    idle(dut);
    dut.io_loadLookupData = kCLwiLoad;
    dut.io_loadLookupWaitBlocked = 0;
    tick(dut);
    ...
    require(completions == 1, "unblocked C.LWI did not complete exactly once");
}

void prove_c_ldi_control(VCompressedWordLoadExecuteProbe &dut) {
    reset(dut);
    drive_load(dut, kOpCLdi, kCLdiImm, kCLdiLoad);
    require(dut.io_loadLookupValid, "C.LDI did not assert loadLookupValid");
    require(dut.io_loadLookupAddr == kCLdiAddr, "C.LDI did not use pre-scaled immediate directly");
    require(dut.io_loadLookupSize == 8, "C.LDI load size was not eight bytes");
    require(!dut.io_loadLookupReturnSignExtend, "C.LDI unexpectedly requested sign extension");
    require_identity_lookup(dut);
    wait_for_completion(dut, "C.LDI did not complete after lookup");
    require_common_completion(dut, kCLdiLoad, kCLdiAddr, 8);
}
...
int main(int argc, char **argv) {
    ...
    prove_c_lwi_first_red(dut);
    prove_c_lwi_wait_hold_once(dut);
    prove_c_ldi_control(dut);

    std::cout << "backend-compressed-word-load-execute-probe: PASS\n";
    return 0;
}
