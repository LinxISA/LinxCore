...
SOURCE_SHAPE_CONTRACTS = {
        "ooo_current": {
            "start_signature": "object OooIexAluPipeline {",
            "end_signature": "\n}\n\n/** One-cycle integer ALU",
            "forbidden_private_def_between_markers": False,
            "expected": {
                "frontend_strict_decode": {"covered": 547, "denominator": 547},
                "reduced_scalar_alu_support": {"covered": 17, "denominator": 547},
                "cross_stack_aligned_support": {"covered": 17, "denominator": 547},
            },
        },
        "expanded_current": {
...
def _extract_is_supported_body(path: Path) -> tuple[str, str, dict[str, Any]]:
    ...
    if start < 0:
        contract = SOURCE_SHAPE_CONTRACTS["ooo_current"]
        start = text.find(contract["start_signature"])
        end = text.find(contract["end_signature"], start)
        if start < 0 or end < 0:
            raise ValueError(
                f"{path}: could not isolate a supported-opcode source-shape contract"
            )
        return "ooo_current", text[start:end], contract
