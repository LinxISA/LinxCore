...
if [[ -n "${FETCH_RF_SEED}" ]]; then
  if [[ "${FETCH_RF_SEED}" != /* ]]; then
    FETCH_RF_SEED="${ROOT_DIR}/${FETCH_RF_SEED}"
  fi
  if [[ ! -f "${FETCH_RF_SEED}" ]]; then
    echo "error: FETCH_RF_SEED does not exist: ${FETCH_RF_SEED}" >&2
    exit 2
  fi
  TB_ARGS+=(--rf-seed "${FETCH_RF_SEED}")
fi
if [[ "${FETCH_MARKER_ROWS_TRACE_TOP}" == "1" ]]; then
...
if [[ "${FETCH_ALLOW_RESIDUAL_REPLAY_LIQ_WAIT}" == "1" ]]; then
  TB_ARGS+=(--allow-residual-replay-liq-wait)
fi
verilator \
  --cc "${SV_FILES[@]}" \
