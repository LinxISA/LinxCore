...
BUILD_ARG="generated/chisel-verilator/backend-reduced-scalar-alu-hl-sdi-pr-probe"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --build-dir)
      BUILD_ARG="$2"
...
verilator --cc "${sv_sources[@]}" --top-module ReducedScalarAluHlSdiPrProbe \
  --exe "${ROOT_DIR}/tools/chisel/backend_reduced_scalar_alu_hl_sdi_pr_probe_tb.cpp" \
  --build --build-jobs 0 -Mdir "${OBJ_DIR}" \
  -o backend_reduced_scalar_alu_hl_sdi_pr_probe_tb -CFLAGS '-std=c++17 -O2'

"${OBJ_DIR}/backend_reduced_scalar_alu_hl_sdi_pr_probe_tb"
