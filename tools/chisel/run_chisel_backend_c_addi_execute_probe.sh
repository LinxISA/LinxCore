...
BUILD_ARG="generated/chisel-verilator/backend-c-addi-execute-probe"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --build-dir)
      if [[ $# -lt 2 ]]; then
...
verilator --cc "${sv_sources[@]}" --top-module ReducedScalarAluHlSdiPrProbe \
  --exe "${ROOT_DIR}/tools/chisel/backend_c_addi_execute_probe_tb.cpp" \
  --build --build-jobs 0 -Mdir "${OBJ_DIR}" \
  -o backend_c_addi_execute_probe_tb -CFLAGS '-std=c++17 -O2'

"${OBJ_DIR}/backend_c_addi_execute_probe_tb"
