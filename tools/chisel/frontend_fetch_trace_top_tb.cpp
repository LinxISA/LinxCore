#include "VLinxCoreFrontendFetchTraceTop.h"
#include "verilated.h"
...
void clear_inputs(VLinxCoreFrontendFetchTraceTop &dut) {
  dut.io_startValid = 0;
  dut.io_startPc = 0;
...
void tick(VLinxCoreFrontendFetchTraceTop &dut) {
  dut.clock = 0;
  dut.eval();
  dut.clock = 1;
  dut.eval();
...
void reset(VLinxCoreFrontendFetchTraceTop &dut) {
  clear_inputs(dut);
  dut.reset = 1;
  tick(dut);
  tick(dut);
...
void expect_monitor_clean(
    const VLinxCoreFrontendFetchTraceTop &dut,
    const char *context,
...
ObservedRow read_slot0(const VLinxCoreFrontendFetchTraceTop &dut) {
  ObservedRow row;
...
ObservedRow read_slot1(const VLinxCoreFrontendFetchTraceTop &dut) {
  ObservedRow row;
...
void expect_row(const ObservedRow &observed, const ExpectedRow &expected) {
  if (!observed.valid ||
    ...
    std::cerr << "frontend fetch trace top commit row mismatch"
  ...
  if (observed.src0_valid &&
    ...
    std::cerr << "frontend fetch trace top src0 mismatch"
  ...
  if (observed.src1_valid &&
    ...
    std::cerr << "frontend fetch trace top src1 mismatch"
  ...
  if (observed.dst_valid &&
    ...
    std::cerr << "frontend fetch trace top dst mismatch"
...
void start_source(VLinxCoreFrontendFetchTraceTop &dut, std::uint64_t pc) {
  clear_inputs(dut);
  dut.io_startValid = 1;
  dut.io_startPc = pc;
  tick(dut);
  ...
  if (!dut.io_sourceActive) {
    std::cerr << "frontend fetch source did not arm at start pc=0x"
              << std::hex << pc << std::dec << "\n";
    std::exit(1);
  }
}
...
std::uint8_t fetch_and_enqueue_row(
    VLinxCoreFrontendFetchTraceTop &dut,
  ...
  for (int cycle = 0; cycle < 8; ++cycle) {
    ...
    if (dut.io_fetchReqValid) {
      if (dut.io_fetchReqPc != row.pc || !dut.io_sourceReqFire) {
        std::cerr << "frontend fetch request mismatch"
  ...
  if (!saw_req) {
    std::cerr << "frontend fetch source did not issue request for pc=0x"
  ...
  if (!dut.io_fetchRespReady || !dut.io_sourceRespFire) {
    std::cerr << "frontend fetch response was not accepted"
  ...
  for (int cycle = 0; cycle < 8; ++cycle) {
    ...
    if (dut.io_sourceOutFire) {
      if (!dut.io_decodeReady || !dut.io_selectedValid ||
        ...
        std::cerr << "frontend fetch packet was not accepted by F4/decode path"
      ...
      for (int rename_cycle = 0; rename_cycle < 8; ++rename_cycle) {
        clear_inputs(dut);
        dut.eval();
      ...
      std::cerr << "frontend fetch row did not reach rename/ROB update"
                << " pc=0x" << std::hex << row.pc << std::dec << "\n";
      std::exit(1);
    }
    tick(dut);
  ...
  std::cerr << "frontend fetch packet source did not emit packet"
            << " pc=0x" << std::hex << row.pc << std::dec << "\n";
  std::exit(1);
}

void complete_rob_row(VLinxCoreFrontendFetchTraceTop &dut, std::uint8_t rob_value) {
  clear_inputs(dut);
...
void drain_empty(VLinxCoreFrontendFetchTraceTop &dut) {
  for (int cycle = 0; cycle < 8; ++cycle) {
    clear_inputs(dut);
    dut.eval();
  ...
  std::cerr << "frontend fetch trace top did not drain after commit"
...
void commit_expected_row(
  ...
  for (int cycle = 0; cycle < 8; ++cycle) {
    ...
    if (dut.io_commit_rows_0_valid) {
      ...
      expect_monitor_clean(dut, "frontend fetch trace top commit", 0x1, 1);
      expect_row(slot0, expected);
      if (slot1.valid) {
        std::cerr << "frontend fetch trace top expected a single-row commit window\n";
        std::exit(1);
      }
    ...
    expect_monitor_clean(dut, "frontend fetch trace top wait", 0x0, 0);
    tick(dut);
  }

  std::cerr << "frontend fetch trace top did not emit a commit row"
...
int main(int argc, char **argv) {
  ...
  dut.eval();
  if (!dut.io_idle || !dut.io_empty || dut.io_size != 0) {
    std::cerr << "frontend fetch trace top did not finish idle"
              << " idle=" << static_cast<unsigned>(dut.io_idle)
              << " empty=" << static_cast<unsigned>(dut.io_empty)
              << " size=" << static_cast<unsigned>(dut.io_size)
              << "\n";
