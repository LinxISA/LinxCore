...
def _format_uop_detail(row: UopRow, opcode: str) -> str:
...
def _format_block_detail(block: BlockState) -> str:
    parts = [
...
def main() -> int:
    ...
    for row in uop_rows:
        ...
        op_name = row.op_name if row.op_name else op_by_seq.get(row.commit_seq or -1, "uop")
        opcode = _extract_opcode(asm, op_name)
        pseudo_asm = _emit_pseudo_asm(opcode, row.commit_payload)
        kind_text = row.kind_text(asm_raw)
        if _is_generated_uop(kind_text, row.parent_uid, asm_raw):
            seq_text = str(row.commit_seq) if row.commit_seq is not None else "-"
            left_label = f"GEN seq={seq_text} op={pseudo_asm}"
            entity_kind = "gen_uop"
        else:
            asm_text = asm if asm else pseudo_asm
            left_label = f"{_pc_label(row.pc, sym_sorted, sym_addrs)}: {asm_text}"
            entity_kind = "uop"
        detail = _format_uop_detail(row, opcode)
    ...
    for block in block_rows:
        ...
        detail = _format_block_detail(block)
        header_records.append(
            {
                "type": "OP_DEF",
                "cycle": bootstrap_cycle,
