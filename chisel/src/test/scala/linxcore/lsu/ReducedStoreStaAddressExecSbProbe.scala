...
class ReducedStoreStaAddressExecSbProbeIO extends Bundle {
...
class ReducedStoreStaAddressExecSbProbe extends Module {
  private val p = InterfaceParams()

  val io = IO(new ReducedStoreStaAddressExecSbProbeIO)
  ...
  bridge.io.queue.uop.insnLen := 4.U
...
object EmitReducedStoreStaAddressExecSbProbe extends App {
  val targetDir = args.sliding(2, 1).collectFirst {
    case Array("--target-dir", dir) => dir
  }.getOrElse("generated/chisel-verilog/lsu-reduced-store-sta-address-exec-sb-probe-r813")

  ChiselStage.emitSystemVerilogFile(
    new ReducedStoreStaAddressExecSbProbe,
