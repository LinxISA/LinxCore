...
class ScalarLSUSpec extends AnyFunSuite {
  ...
  test("Chisel ScalarLSU owns the parameterized STQ-to-SCB store path") {
    ...
    assert(sv.contains("io_load_loadReturn_resolveReady"))
    assert(sv.contains("io_load_loadReturn_completionSelectedPipe"))
    assert(sv.contains("io_load_loadReturn_resolveFire"))
