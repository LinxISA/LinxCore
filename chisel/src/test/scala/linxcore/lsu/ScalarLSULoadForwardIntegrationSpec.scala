...
class ScalarLSULoadForwardIntegrationHarness extends Module {
  ...
  path.io.alloc.returnPipeIndex := 2.U
  ...
  val capturedQuery = Reg(chiselTypeOf(forward.queries(2).bits))
  when(forward.queries(2).fire) {
    capturedQuery := forward.queries(2).bits
  }
  forward.responses(2).valid := io.responseValid
  forward.responses(2).bits.query := capturedQuery
  forward.responses(2).bits.loadByteMask := "hff".U
  forward.responses(2).bits.forwardMask := "hff".U
  forward.responses(2).bits.mergedLineData := "h8877665544332211".U
  forward.responses(2).bits.bypassComplete := true.B

  io.allocAccepted := path.io.allocAccepted
  io.launchAccepted := path.io.launchAccepted
  io.queryValid := forward.queries(2).valid
  io.queryAccepted := forward.queryAccepted
  io.queryLoadId := forward.queries(2).bits.loadId
  io.queryAttempt := forward.queries(2).bits.attempt
  io.responseReady := forward.responses(2).ready
...
class ScalarLSULoadForwardIntegrationSpec extends AnyFunSuite with ChiselSim {
  ...
  test("hard recovery clears a retained STQ query before external acceptance") {
    simulate(new ScalarLSULoadForwardIntegrationHarness) { dut =>
      dut.io.flush.poke(false.B)
      dut.io.preciseFlush.poke(false.B)
      dut.io.nativeBid.poke(5.U)
  ...
  test("typed precise recovery fences but preserves a surviving retained query") {
    simulate(new ScalarLSULoadForwardIntegrationHarness) { dut =>
      dut.io.flush.poke(false.B)
      dut.io.preciseFlush.poke(false.B)
      dut.io.nativeBid.poke(5.U)
  ...
  test("typed precise recovery preserves an E4 result for exact post-recovery apply") {
    simulate(new ScalarLSULoadForwardIntegrationHarness) { dut =>
      dut.io.flush.poke(false.B)
      dut.io.preciseFlush.poke(false.B)
      dut.io.nativeBid.poke(5.U)
  ...
  test("an unrepresentable native BID stalls and reports instead of aliasing") {
    simulate(new ScalarLSULoadForwardIntegrationHarness) { dut =>
      dut.io.flush.poke(false.B)
      dut.io.preciseFlush.poke(false.B)
      dut.io.nativeBid.poke(13.U)
      dut.io.allocValid.poke(false.B)
      dut.io.launchValid.poke(false.B)
      dut.io.queryReady.poke(true.B)
      dut.io.responseValid.poke(false.B)
