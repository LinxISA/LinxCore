...
class LoadReplayReturnPipeW1SlotSpec extends AnyFunSuite {
  import LoadReplayReturnPipeResidencySlotReference._

  test("captures an advanced scalar LDA payload into an empty W1 slot") {
  ...
  test("captures a vector AGU target and blocks a second W1 write while occupied") {
    val first = step(
      State(),
      enable = true,
      flush = false,
  ...
  test("reports disabled no-write and invalid-target W1 blockers") {
    val disabled = step(
      State(),
      enable = false,
      flush = false,
  ...
  test("Chisel LoadReplayReturnPipeW1Slot elaborates W1 payload registers and blockers") {
    val sv = ChiselStage.emitSystemVerilog(
      new LoadReplayReturnPipeW1Slot(returnPipeCount = 2))
