...
class ScalarLSULoadPathSpec extends AnyFunSuite {
  ...
  test("ScalarLSULoadPath elaborates LIQ-to-ResolveQ lifecycle ownership") {
    ...
    assert(sv.contains("io_loadReturn_completionCandidateValid"))
    assert(sv.contains("io_loadReturn_completionSelectedPipe"))
    assert(sv.contains("io_loadReturn_resolveFire"))
    assert(sv.contains("io_loadReturn_writebackFire"))
    assert(sv.contains("io_loadReturn_wakeupFire"))
