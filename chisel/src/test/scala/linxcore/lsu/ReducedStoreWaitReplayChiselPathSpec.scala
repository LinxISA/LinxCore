...
class ReducedStoreWaitReplayChiselPathSpec extends AnyFunSuite {
  ...
  test("Chisel wait-replay path probe elaborates the real STQ to LIQ owner chain") {
    ...
    assert(sv.contains("io_liqReplayWakeValid"))
    assert(sv.contains("io_liqReplayWake_source"))
    assert(sv.contains("io_liqReplayWakeCompletedMask"))
    ...
    assert(sv.contains("io_resolveQueueRetireValid"))
    assert(sv.contains("io_resolveQueueRetireCount"))
    assert(sv.contains("io_resolveQueueFirstLoadLsId_value"))
    assert(sv.contains("MDBConflictDetect"))
    assert(sv.contains("io_mdbStore_valid"))
    assert(sv.contains("io_mdbResolveCandidateMask"))
    assert(sv.contains("io_mdbNukeFlush"))
    assert(sv.contains("io_mdbConflictLoadLsId_value"))
    assert(sv.contains("MDBQueueFanout"))
    ...
    assert(sv.contains("io_mdbDeleteValid"))
    assert(sv.contains("io_mdbFanoutDeleteAccepted"))
    assert(sv.contains("io_mdbFanoutDeleteReleased"))
    assert(sv.contains("io_mdbFanoutLuOutHit"))
    assert(sv.contains("io_mdbFanoutSuWakeupValid"))
    assert(sv.contains("LoadReplayMdbLookupWaitPlan"))
    assert(sv.contains("io_mdbLookupUseLiveLoad"))
    assert(sv.contains("io_mdbLookupWaitPlanBlockedByNoTarget"))
    ...
    assert(sv.contains("io_liqWaitStoreMask"))
    assert(sv.contains("io_liqScbReturnedMask"))
