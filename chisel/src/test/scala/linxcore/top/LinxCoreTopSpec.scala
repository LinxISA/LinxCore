...
class LinxCoreTopSpec extends AnyFunSuite {
  ...
  test("Chisel LinxCoreTop elaborates with commit and scalar LSU owners") {
    ...
    assert(sv.contains("io_scalarLsu_load_loadReturn_robLookupRid_value"))
    ...
    assert(sv.contains("io_scalarLsu_recovery_source_blockBid"))
    assert(sv.contains("io_scalarLsu_recovery_sourceReady"))
    assert(sv.contains("io_scalarLsu_recovery_sourceStidInRange"))
