...
class ReducedScalarAluSbProbeIO extends Bundle {
  ...
  val completeSrc2PhysValid = Output(Bool())
  val completeSrc2PhysTag = Output(UInt(6.W))
...
class ReducedScalarAluSbProbe extends Module {
  ...
  val io = IO(new ReducedScalarAluSbProbeIO)
  ...
  execute.io.in.opcode := FrontendOpcodeDecodeTable.OP_SB.U
  ...
  execute.io.in.src(1).archTag := io.srcLArchTag
  execute.io.in.src(1).physTag := io.srcLPhysTag
  execute.io.in.src(1).ready := true.B
  execute.io.in.src(2).valid := true.B
  execute.io.in.src(2).operandClass := OperandClass.P
  execute.io.in.src(2).archTag := io.srcRArchTag
  execute.io.in.src(2).physTag := io.srcRPhysTag
  execute.io.in.src(2).ready := true.B
  execute.io.in.dst(0).valid := false.B

  execute.io.srcData(0) := io.srcDData
  execute.io.srcData(1) := io.srcLData
  execute.io.srcData(2) := io.srcRData
  ...
  io.completeSrc2PhysValid := execute.io.completeSrcPhysValid(2)
  io.completeSrc2PhysTag := execute.io.completeSrcPhysTag(2)
...
object EmitReducedScalarAluSbProbe extends App {
  val targetDir = args.sliding(2, 1).collectFirst {
    case Array("--target-dir", dir) => dir
  }.getOrElse("generated/chisel-verilog/backend-reduced-scalar-alu-sb-probe")

  ChiselStage.emitSystemVerilogFile(
    new ReducedScalarAluSbProbe,
