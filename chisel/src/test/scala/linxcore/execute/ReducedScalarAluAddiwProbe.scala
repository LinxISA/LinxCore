...
class ReducedScalarAluAddiwProbeIO extends Bundle {
  val inValid = Input(Bool())
  val pc = Input(UInt(64.W))
  val insnRaw = Input(UInt(64.W))
  val uimm12 = Input(UInt(12.W))
...
class ReducedScalarAluAddiwProbe extends Module {
  ...
  val io = IO(new ReducedScalarAluAddiwProbeIO)
  ...
  execute.io.in.opcode := FrontendOpcodeDecodeTable.OP_ADDIW.U
  execute.io.in.dispatchTarget := DispatchTarget.Alu
  execute.io.in.imm := io.uimm12
...
object EmitReducedScalarAluAddiwProbe extends App {
  val targetDir = args.sliding(2, 1).collectFirst {
    case Array("--target-dir", dir) => dir
  }.getOrElse("generated/chisel-verilog/backend-reduced-scalar-alu-addiw-probe")

  ChiselStage.emitSystemVerilogFile(
    new ReducedScalarAluAddiwProbe,
