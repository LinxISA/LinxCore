...
object TULinkRenameReference {
  final case class Entry(valid: Boolean, bid: ROBIDValue, rid: ROBIDValue, gid: ROBIDValue, seq: ROBIDValue, physTag: Int)
  ...
  def initial(mapQDepth: Int): BankState =
    BankState(
      ...
      usedEntries = 0,
      usedPhys = 0,
      mapQ = Vector.fill(mapQDepth)(Entry(valid = false, ROBIDValue(), ROBIDValue(), ROBIDValue(), ROBIDValue(), 0)))
  ...
  def allocate(state: BankState, bid: ROBIDValue, rid: ROBIDValue, gid: ROBIDValue, localRegs: Int): (BankState, Int, ROBIDValue) = {
    ...
    val next = state.copy(
      ...
      mapQ = state.mapQ.updated(seq.value, Entry(valid = true, bid, rid, gid, seq, phys))
    )
    (next, phys, seq)
  }
