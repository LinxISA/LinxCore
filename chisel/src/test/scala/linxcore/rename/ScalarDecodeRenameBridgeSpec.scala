...
class ScalarDecodeRenameBridgeSpec extends AnyFunSuite {
  ...
  test("IO exposes decode-to-rename, ROB allocation, and cleanup handoff signals") {
    ...
    assert(io.in.prediction.predictionTag.getWidth == 64)
    assert(io.out.prediction.predictionTag.getWidth == 64)
    assert(io.out.prediction.provider.getWidth == 4)
    assert(io.out.prediction.stage.getWidth == 3)
