...
class InterfaceBundlesSpec extends AnyFunSuite {
  ...
  test("decoded uop packet matches the canonical D2 architectural shape") {
    ...
    assert(decoded.isLoad.getWidth == 1)
    assert(decoded.isStore.getWidth == 1)
    assert(decoded.storeSplitIntent.getWidth == 1)
    assert(decoded.isLoadStorePair.getWidth == 1)
    assert(decoded.isStorePcr.getWidth == 1)
    assert(decoded.cacheMaintainNoSplit.getWidth == 1)
    assert(decoded.isLastInBlock.getWidth == 1)
  ...
  test("renamed, issue, LSU, ROB, and trace packets keep model identity and block BID separate") {
    ...
    assert(renamed.isLoad.getWidth == 1)
    assert(renamed.isStore.getWidth == 1)
    assert(renamed.storeSplitIntent.getWidth == 1)
    assert(renamed.isLoadStorePair.getWidth == 1)
    assert(renamed.isStorePcr.getWidth == 1)
    assert(renamed.cacheMaintainNoSplit.getWidth == 1)
    assert(renamed.isLastInBlock.getWidth == 1)
