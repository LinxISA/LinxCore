...
final case class OooParams(
  ...
  require(memoryTransactionIdWidth > 0 &&
    memoryTransactionGenerationWidth > 0,
    "memory transactions need positive value and generation widths")
  ...
  def robPhysicalMemberIndexWidth: Int = math.max(1,
    log2Ceil(maxOrdinaryUopsPerGroup))
  ...
  def toMainline: linxcore.params.OOOParams =
    linxcore.params.OOOParams(
      ...
      pcOffsetWidth = pcOffsetWidth,
      pcWritePorts = pcWritePorts,
      pcReadPorts = pcReadPorts,
      pcReadReplicaCount = pcReadReplicaCount,
...
object OooParams {
  ...
  def fromMainline(p: linxcore.params.OOOParams): OooParams = {
    ...
    OooParams(
      ...
      pcOffsetWidth = p.pcOffsetWidth,
      pcWritePorts = p.pcWritePorts,
      pcReadPorts = p.pcReadPorts,
      pcReadReplicaCount = p.pcReadReplicaCount,
  ...
  def fromCoreParams(p: linxcore.params.CoreParams): OooParams =
    fromMainline(p.ooo).copy(
      ...
      pcBufferEntries = p.ooo.pcBufferEntries,
      pcBankCount = p.ooo.pcBankCount,
      pcRecoveryScanGroupsPerCycle =
        p.ooo.pcRecoveryScanGroupsPerCycle,
      pcOffsetWidth = p.ooo.pcOffsetWidth,
      pcWritePorts = p.ooo.pcWritePorts,
      pcReadPorts = p.ooo.pcReadPorts,
      pcReadReplicaCount = p.ooo.pcReadReplicaCount,
      reservationEpochWidth = p.ooo.pcAllocationEpochWidth)
}
