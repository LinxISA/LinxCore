...
final case class ScalarLsuParams(
  ...
  require(loadRefillQueueEntries > 1 && (loadRefillQueueEntries & (loadRefillQueueEntries - 1)) == 0,
    "loadRefillQueueEntries must be a power of two greater than one")
...
object ScalarLsuParams {
  ...
  def fromMainline(p: linxcore.params.CoreParams): ScalarLsuParams =
    ScalarLsuParams(
      ...
      resolveQueueEntries = p.lsu.resolveQueueEntries,
      mdbSsitEntries = p.lsu.mdbSsitEntries,
      mdbCommandQueueEntries = p.lsu.mdbCommandQueueEntries,
      mdbOutputQueueEntries = p.lsu.mdbOutputQueueEntries,
      mdbWaitPlanQueueEntries = p.lsu.mdbWaitPlanQueueEntries,
      mdbRecoveryQueueEntries = p.lsu.mdbRecoveryQueueEntries,
      mdbFailedWaitTimeoutCycles = p.lsu.mdbFailedWaitTimeoutCycles,
      loadReturnQueueEntries = p.lsu.loadReturnQueueEntries,
      loadReturnPipeCount = p.lsu.loadPipes,
      l1dSets = p.lsu.l1dSets,
      l1dWays = p.lsu.l1dWays,
