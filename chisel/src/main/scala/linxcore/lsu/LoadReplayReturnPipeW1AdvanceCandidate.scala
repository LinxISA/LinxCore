...
class LoadReplayReturnPipeW1AdvanceCandidateIO(val returnPipeCount: Int = 1)
    extends Bundle {
...
class LoadReplayReturnPipeW1AdvanceCandidate(val returnPipeCount: Int = 1)
    extends Module {
  require(returnPipeCount > 0, "returnPipeCount must be positive")

  private val returnPipeIndexWidth = math.max(1, log2Ceil(returnPipeCount))

  val io = IO(new LoadReplayReturnPipeW1AdvanceCandidateIO(returnPipeCount))
