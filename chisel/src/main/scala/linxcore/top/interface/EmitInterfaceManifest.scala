...
object InterfaceManifest {
  ...
  private val endpointDefinitions = Seq(
    ...
    EndpointDefinition("ooo_to_top_system_issue", "IFC-OOO-IEX-001", "OOO", "TOP",
      _.iex.systemMulticycleQueues, p => new SystemIssueTxn(p)),
    ...
    EndpointDefinition("lsu_to_iex_load_allocation_preview", "IFC-IEX-LSU-001",
      "LSU", "IEX", _.lsu.loadPipes, p => new LoadAllocationPreview(p)),
    EndpointDefinition("lsu_to_iex_load_launch", "IFC-IEX-LSU-001", "LSU", "IEX",
      _.lsu.loadPipes, p => new LoadLaunchTxn(p)),
    EndpointDefinition("iex_to_lsu_store_reservation", "IFC-IEX-LSU-001",
      "IEX", "LSU", _.lsu.storePipes, p => new StoreReservationTxn(p)),
    ...
    EndpointDefinition("ooo_to_lsu_store_commit", "IFC-IEX-LSU-001", "OOO", "LSU",
      _ => 1, p => new StoreCommitAuthorizationTxn(p)),
    EndpointDefinition("lsu_internal_store_classify", "IFC-IEX-LSU-001",
      "LSU Translation", "LSU Commit Backend", _ => 1,
      p => new StoreMemoryClassifyTxn(p)),
    ...
    EndpointDefinition("external_to_dtu_debug_request", "IFC-DTU-001", "External Debug",
      "DTU", _ => 1, p => new DebugRequest(p)),
    EndpointDefinition("dtu_to_ooo_debug_request", "IFC-DTU-001", "DTU", "OOO",
      _ => 1, p => new DebugRequest(p)),
    EndpointDefinition("ooo_to_dtu_debug_response", "IFC-DTU-001", "OOO", "DTU",
      _ => 1, p => new DebugResponse(p)),
    EndpointDefinition("dtu_to_external_debug_response", "IFC-DTU-001", "DTU",
      "External Debug", _ => 1, p => new DebugResponse(p)),
    EndpointDefinition("dtu_trace_export", "IFC-DTU-001", "DTU", "External Trace",
      _.dtu.traceWidth, p => new TracePacket(p)),
    EndpointDefinition("dtu_performance_counter", "IFC-DTU-001", "DTU",
      "External Counter", _.dtu.performanceCounterCount, _ => UInt(64.W)),
