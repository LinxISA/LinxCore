...
```mermaid
flowchart LR
  BCC["BCC<br/>block control + scalar OoO"] --> Cube["Cube Core"]
  BCC --> Vector["Vector Core"]
  BCC --> TMA["TMA"]
  Cube <-->|"256 B"| TileReg["Tile Register"]
  Vector <-->|"256 B"| TileReg
  TMA <-->|"256 B"| TileReg
  TMA <-->|"256 B"| L2["L2 Cache"]
  BCC <-->|"64 B"| L1["L1 Cache"]
  L1 <--> L2
```
...
```mermaid
flowchart LR
  subgraph FE["SMT Fetch"]
    PC["Thread PC / predictor"]
    IC["iTLB + I-cache"]
    IB["INST_BUF / CT"]
    PC --> IC --> IB
  end

  subgraph BE["Decode, Rename and Dispatch"]
    DEC["4-wide Decode"]
    REN["GPR / ClockHands Rename"]
    ROB["PE ROB"]
    DEC --> REN
    REN --> ROB
  end
  ...
  IB --> DEC
  REN --> AB["AB ISQ<br/>ALU / branch"]
  REN --> AM["AM ISQ<br/>MUL / complex ALU"]
  REN --> LS["LS0 / LS1<br/>scalar LSU"]
  REN --> CMD["CMD_ISQ<br/>B.IOR / B.IOT / B.DIM"]
  REN --> BROB["BROB"]
  CMD --> TR["TileRename"]
  TR --> BISQ["Cube / Vector / TMA BISQ"]
  BISQ --> PE["Cube / Vector / TMA"]
```
...
// TMA block
BSTART.TMA MGATHER, U8
B.IOT  [M#1], last -> M<32KB>
B.DIM  zero, 4096 -> lb0
B.DIMI 1          -> lb1

// Cube block
BSTART.CUBEMATMULMX, e1m2x2
B.IOT  [T#7, U#2]
B.IOT  [T#1, M#1], last -> acc<64KB>
C.B.DIMI 128 -> lb0
C.B.DIMI 128 -> lb1
C.B.DIMI 256 -> lb2
...
| Tile tag / entry index | Base offset | Size in 128 B units |
| --- | ---: | ---: |
| 0 | 0 | 4 |
| 1 | 4 | 8 |
| 2 | 12 | 16 |
...
| TileRename no credit | B.IOT/CMD pipeY | scalar compute、scalar LSU、其他 ready BISQ |
