...
| `vec_cmd_stid` | `STID_W` | input | 选择 per-STID BROB ring |
| `vec_cmd_bid` | `BID_W` (default 8) | input | 该 STID ring 的 Block ID，用于完成回报以及 BCC/BROB 侧匹配 |
...
| `vec_done_stid` | `STID_W` | output | 对应的 BROB ring |
| `vec_done_bid` | `BID_W` | output | 对应的 Block ID；BCC/BROB 使用 `(STID,BID)` 标记块完成 |
...
| `cmd_stid` | `STID_W` | 当前块的 BROB ring |
| `cmd_bid` | `BID_W` (default 8) | 当前块在该 ring 的 BID |
...
Cycle M+1:  Vector Core 向 BCC 回报 vec_done_valid=1, vec_done_stid=cmd_stid, vec_done_bid=cmd_bid
```
