...
预取请求生成  █
              ↓
发送 flit #1      █────────────→ TMU 处理 (4cy)
                                    ↓
发送 flit #2          █────────────→ TMU 处理 (4cy)
                                         ↓
                                    返回 flit #1
                                         ↓
                                    返回 flit #2
                                         ↓
L0 Cache 填充完成                          █ (512B ready)

总延迟：~6 cycles (从请求到数据就绪)
```
...
关键路径:
  无RMW: TReg(5) + L0(1) + MAC(7) + Buf(1) = 14 cy
  有RMW: TReg(5) + L0(1) + MAC(7) + Buf(1) + ACC(9) = 23 cy
```
