...
| LinxCore | `LinxCore::BROB order-state probe` | `bash rtl/LinxCore/tools/chisel/run_chisel_brob_order_state_probe.sh` | generated Chisel proof for independent per-STID allocation/commit/count windows, exact-head completion ordering, irrevocable retire backpressure, fair cross-STID retirement, simultaneous allocate/retire, validated recovery, modular metadata reuse, exact strong non-flush prefixes across rollover, unsafe-head blocking, and exception exclusion |
