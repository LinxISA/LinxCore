...
| LinxCore | `LinxCore::MDB recovery to ROB probe` | `bash rtl/LinxCore/tools/chisel/run_chisel_recovery_cleanup_rob_probe.sh` | non-oldest retention, wrong-RID lookup retention, exact allocator-stamped full-BID recovery, retained multi-source same-STID oldest selection, invalid-STID rejection, cross-STID fairness, consume-plus-replace, accepted block authority, and scoped resident ROB prune |
