...
| Load forwarding/replay | `LoadStoreForwarding`, `LoadForwardPipeline`, `LoadInflightQueue`, `LoadMissQueue`, `LoadRefillTransport`, `LoadResolveQueue`, `ScalarLSULoadPath`, `LoadRefillWakeup`; verify exact miss-response retention, dual-ingress FIFO order, post-dequeue refill credit, stable blocked output, hard clear, and typed-recovery hold |
