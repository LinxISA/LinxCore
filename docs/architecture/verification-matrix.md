...
| ROB/BROB/recovery | `ROBEntryBank`, `ROBRecoveryWatermark`, `ROBFlushPrune`, `ReducedCommitROB`, `BROB`, `BrobLiveBidResolver`, `BrobOrderState`, `FullBidRecoveryBridge`, `ScalarLSURecoverySource`, `ScalarRedirectRecoverySource`, `RecoveryProducerQueue`, `BccRecoverySource`, `IexSlowInsertRecoverySource`, `IexIqStallRecoverySource`, `PeMismatchRecoverySource`, `RecoveryProvenance`, `RecoverySourceArbiter`, `RecoveryClassMerge`, `RecoveryFabric`, `RecoveryBackendControl`, `DecodeRenameROBPath`; verify per-STID circular RID selection, retired-row exclusion, exact ROB/BROB watermark matching, canonical BID slot resolution inside the owner head/live-count window, retained producer backpressure, typed event mapping, accepted-intent-only state mutation, cause resolution, matched payload-sidecar authorization, and missing-identity blocking; 64-bit upper BID transport remains diagnostic legacy until removed |
