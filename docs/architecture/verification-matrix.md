...
| ROB/BROB/recovery | `ROBEntryBank`, `ROBFlushPrune`, `ReducedCommitROB`, `BROB`, `FullBidRecoveryBridge`, `ScalarLSURecoverySource`, `ScalarRedirectRecoverySource`, `RecoveryProducerQueue`, `BccRecoverySource`, `IexSlowInsertRecoverySource`, `IexIqStallRecoverySource`, `PeMismatchRecoverySource`, `RecoveryProvenance`, `RecoverySourceArbiter`, `RecoveryClassMerge`, `RecoveryFabric`, `RecoveryBackendControl`, `DecodeRenameROBPath`; verify exact owner-supplied full BID, retained producer backpressure, typed event mapping, accepted-intent-only state mutation, cause resolution, matched payload-sidecar authorization, and missing-identity blocking; 64-bit/upper-uniqueness BID helpers are legacy until migrated |
