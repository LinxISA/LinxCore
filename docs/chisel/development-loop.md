...
The active LSU priority is I0.15c-b3 live scalar-LSU composition. The b3c3c
packet installs one private retained structural-block policy. Exact
unknown-older dependencies rebind the OOO metadata and LIQ owners under a new
attempt and exact store wait key; stale snapshots retry without a wait key.
Ambiguous authority, malformed/cross-line cases remain fail closed, and the raw
hard-block channel no longer escapes the production wrapper. Typed recovery of
the policy is exact-dual-kill only and still obeys the all-other-LSU-owner-empty
fence. The b3c3b production wrapper installs the b3c3a scalar-load attachment
against the existing execution-cluster metadata owner and existing STQ owner.
It keeps all three forwarding lanes and prospective MDB admission private, proves exactly
...
passes. The next packet must replace the migration BID projection with shared
BID/BROB ordering authority, extend recovery projection equality to the
remaining LSU owners, and implement cross-line forwarding/execution.
...
metadata, canonical 2/4/6-wide D1 decode, exact P/T/U and pair operand
...
O6.1 adds a typed retained fast-resolve owner. Boundary metadata,
SETRET/START_CALL results, precise traps, and no-effect rows are admitted only
...
`docs/chisel/integrated-development-flow.md`. The production graph now includes
the canonical `LinxCoreIfu`, tagged line-memory transport, fixed-width
four-wide D1 decode, exact backend feedback/recovery, direct D1-to-rename/ROB
ingress, and natural CoreMark/Dhrystone benchmark promotion. The open IFU work
is complete TAGE provider/alternate/usefulness policy, path-history/loop
checkpoint recovery, independent B-F4 rank coverage, four-row atomic D2/D3
admission, lower-memory error termination, and SoC PTW/L1I integration. New agents must
...
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh --out-dir generated/r100-live-qemu-fixture` Provenance: source commit 77001f0ec3b463950c1a275fcb5e3dc1638d73cd. <!-- task15-historical-specialized-evidence:end -->
and
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `run_chisel_frontend_fetch_rf_alu_qemu_elf_xcheck.sh --elf generated/r100-live-qemu-fixture/frontend_fetch_rf_alu_qemu_fixture.elf --expected-rows 3 --capture-rows 3 --pc-lo 0x10002 --pc-hi 0x1000b --max-seconds 5` Provenance: source commit 77001f0ec3b463950c1a275fcb5e3dc1638d73cd. <!-- task15-historical-specialized-evidence:end -->
...
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh --out-dir generated/r101-live-qemu-fixture` Provenance: source commit 7eecd8e2770658c633bd52e65654a92d61f7b4e0. <!-- task15-historical-specialized-evidence:end -->
and
...
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh --out-dir generated/r102-live-qemu-fixture` Provenance: source commit d56ea4f505b0e4db9dcd81604e8fdd19bf1ce965. <!-- task15-historical-specialized-evidence:end -->,
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `BUILD_DIR=generated/r102-default-fetch-rf-alu-trace-top-xcheck bash tools/chisel/run_chisel_frontend_fetch_rf_alu_trace_top_xcheck.sh` Provenance: source commit d56ea4f505b0e4db9dcd81604e8fdd19bf1ce965. <!-- task15-historical-specialized-evidence:end -->,
and
...
control. Evidence:
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh --out-dir generated/r104-live-qemu-fixture` Provenance: source commit b487843427125df2aca2284bd17e0578f31e32e0. <!-- task15-historical-specialized-evidence:end -->
and
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `run_chisel_frontend_fetch_rf_alu_qemu_elf_xcheck.sh --build-dir generated/r104-marker-lifecycle-qemu-elf-xcheck --elf generated/r104-live-qemu-fixture/frontend_fetch_rf_alu_qemu_fixture.elf --expected-rows 0 --capture-rows 5 --allow-block-markers --max-seconds 5` Provenance: source commit b487843427125df2aca2284bd17e0578f31e32e0. <!-- task15-historical-specialized-evidence:end -->
...
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh --out-dir generated/r105-long-body-fixture --long-body` Provenance: source commit 96bf4fc077ec9df6759a723c3caf5ffc13154d98. <!-- task15-historical-specialized-evidence:end -->
and
...
R117 started from `linx-isa` commit
`53c9225e63ea404061bc2b69d44ade0a4178d8b4`, `rtl/LinxCore` commit
`198c8641c105ad4674ebe4ee48bbecce311ff666`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`c561976c60fa5f76f00987563772283a4f2d9b97`, and
`skills/linx-skills` commit
`2e3d1ba5e6bc36b25eb1b840583abcef1af14825`. R117 extends the live reduced
RF/ALU CoreMark prefix across the zero-advance `C.BSTART` artifact at
`pc=0x4000554e` and through the next dense scalar/local packet: `C.MOVR`
(`pc=0x40005550`, `insn=0xf886`) moves scalar x2 into T tag `31`, the
following `ADDI` (`pc=0x40005552`, `insn=0x008c0115`) reads encoded `SrcL=T0`
and writes scalar x2, `C.LDI` (`pc=0x40005556`, `insn=0xf61a`) reads local T0
with signed doubleword byte offset `simm5_11 << 3` and emits a zero-data load
from `0x4fff0000`, and `C.SETC_NE` (`pc=0x40005558`, `insn=0x2e36`) validates
local/scalar compare sources while producing no destination or writeback row.
The packet then reaches `C.BSTART.STD.FALL` at `pc=0x4000555a`.

The reduced backend fixes two live CoreMark path hazards found while promoting
that packet. First, reduced marker/block retire now feeds the scalar rename
commit path by converting the retiring full BID back to a ROB id, so scalar
GPR mapQ entries are released at marker-owned block boundaries. Second, the
T/U retire serializer now drains terminal rename responses
(`accepted | miss | releaseMismatch | unsupported`) rather than only
...
`insn=0x13808315`, likely `ADDTPC`, followed by `0x10000395`, `0x4146`, and a
marker at `0x40005566`.

R117 closeout: `skill-evolve: update linx-core (C.LDI scaled local-base,
C.SETC_NE no-writeback, scalar block-commit feedback, terminal T/U retire
command drain)`.

R118 started from `linx-isa` commit
`df4357d5b7c922458c3745f98cfb3e5c641aaac4`, `rtl/LinxCore` commit
`c6121aabdb9a7a3962045123d366f380c983bc8b`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`c561976c60fa5f76f00987563772283a4f2d9b97`, and
`skills/linx-skills` commit
`8935e48b051dd818a6f3c586a64023e51e37812a`. R118 extends the live reduced
RF/ALU CoreMark prefix through the first ordinary scalar store-immediate row,
`OP_SDI` at `pc=0x4000556a`, `insn=0x0182b059`. The model/QEMU contract is
store data from encoded `SrcL`, address base from encoded `SrcR`, and address
offset `simm12_7_s5_25_7 << 3`; the observed row uses visible scalar x5 store
data and suppressed local T0 as the address base. A 41-row capture cuts inside
...
redirecting marker at `pc=0x40005574`.

R118 closeout: `skill-evolve: update linx-core (ordinary SDI store-immediate
row data/base/offset contract and dense-safe 42-row CoreMark gate)`.

R119 started from `linx-isa` commit
`ceb9a625b5f338a5e09599d8ea1361fbf7758c5c`, `rtl/LinxCore` commit
`b1a2039c4dc02704974f36d122625d822bf75e0b`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`1ca8f47999926906e49c124fb9320ef920f42aee`. R119 extends the live reduced
RF/ALU CoreMark prefix through the conditional `C.BSTART` loop edge at
`pc=0x40005574` and one fall-through loop iteration. The reduced execute owner
publishes `C.SETC_NE` as a validity-masked branch decision, the top latches the
decision until the following marker boundary, and `DecodeRenameROBPath` either
allocates the fallthrough marker block or redirects to the active conditional
...
R121 started from `linx-isa` commit
`e32312ab74bb1b2f84a24a15061df03776b6509c`, `rtl/LinxCore` commit
`d1386dcab31ca6495c1bac84ed04d7c65bd43e34`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`d52bdae65891ed1a0038e131ab2d41c9d05c3dd8`. R121 extends the live reduced
RF/ALU CoreMark prefix through the R120 256-row unsupported frontier:
`OP_LDI` at `pc=0x40005576`, the following compact `OP_C_SETC_EQ` equality
compare at `pc=0x4000557c`, and repeated store-loop rows through a 256-row
capture. The first 256-row rerun exposed a harness-only tail issue: the QEMU
capture ended at `pc=0x400055ba` inside an 8-byte F4 window, while the DUT
...
R121 closeout: `skill-evolve: update linx-core (LDI/C.SETC_EQ reduced
CoreMark rows and bounded-capture tail-prefix gate policy)`.

R122 started from `linx-isa` commit
`825178dd4762f69d14adca77046f0eaac5056cf9`, `rtl/LinxCore` commit
`61a2bd17fc7b974a4f72b795c60b40daf9460c2d`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`fb71c4e8a6357c04f516f71cec8e12a4cef1f6cb`. R122 extends the live reduced
RF/ALU CoreMark prefix past the nonzero `OP_LDI` row at `pc=0x400055c2` by
adding a read-only sparse-ELF load lookup for `OP_C_LDI`/`OP_LDI`. It also
admits the next QEMU row shapes discovered by the larger probe:
`OP_SETC_LTU` no-writeback unsigned branch sideband, `ADDI` to T alias,
`C.MOVR` from local T0 to scalar x7, and `LDI` to U alias. Evidence:
...
allocation mismatch at `pc=0x400055d4`; the next packet must settle that
marker policy before adding indexed store memory mutation.

R122 closeout: `skill-evolve: update linx-core (read-only sparse-ELF load
lookup bridge, local-alias row reducer policy, and redirect-marker/OP_SD next
frontier)`.

R123 started from `linx-isa` commit
`7db96a37ffa7d9ad4ef90cc89c6157fb8fed31ee`, `rtl/LinxCore` commit
`25367127b0674d74a6c35d19adaee3d645b2c737`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`7cd1f1be9ba850c8ccb507b5a518262ef815507f`. R123 fixes the redirecting
marker frontier by distinguishing the active block's boundary kind from the
new marker's kind: active direct/call blocks redirect unconditionally to their
recorded target at the next marker boundary and suppress allocation of that
boundary marker, while active conditional blocks still use the SETC result
directly. It also updates the live RF/ALU Verilator harness to collect legal
...
R123 closeout: `skill-evolve: update linx-core (active direct/call marker
boundary redirect policy and live RF/ALU two-slot commit collection)`.

R124 started from `linx-isa` commit
`88387b9f215f6162339516dcc24de8220b1bad4a`, `rtl/LinxCore` commit
`8bfd505604f3fb035897356556aece8f0ab3debf`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`bb4476b563685f62b91728814c23b9380113b5a6`. R124 adds the indexed
`OP_SD` store row at `pc=0x400055f2`. The model-derived source contract is
`SrcL` base, `SrcR` index, and `SrcD` store payload, so the reduced execute
owner emits a no-writeback 8-byte store sideband at
`base + (index << 3)` with data from `srcData(2)`. The live Verilator harness
now mutates the sparse memory image only after a committed 8-byte store row
matches QEMU, letting later reduced load lookups observe program-order store
effects without claiming a full LSU, STQ, cache, or store-forwarding path.
Evidence: `python3 tools/chisel/frontend_fetch_rf_alu_qemu_rows.py --self-test`,
...
R124 closeout: `skill-evolve: update linx-core (OP_SD indexed-store source
mapping and committed sparse-memory store mutation policy)`.

R125 started from `linx-isa` commit
`49457e43edbdb97264f8f9d31341eb74dbe57c9a`, `rtl/LinxCore` commit
`06e3855d713aa03a63d6dea70988f3b3a24bbcf5`,
`model/LinxCoreModel` commit
`1993e4e749403824a4908548baf77d5e15117068`, QEMU commit
`70853e25c11398f33a7e7269e8718f05a36f975b`, and
`skills/linx-skills` commit
`12ad59b74564ab8d82e49cf2013e17f228e416e6`. R125 extends the reduced
RF/ALU CoreMark envelope through a 1024-row live QEMU capture. It adds
`OP_SUBI`, compressed local `OP_C_AND`, encoded local/scalar `OP_ADD`
validation, and compressed `OP_C_SDI` store-immediate sideband handling.
`C.AND` uses the same model-derived implicit T writeback synthesis already
used for QEMU trace-gap `C.ADD` rows. `C.SDI` validates the compressed base
plus signed bits `[15:11] << 3`, fixed T0 store payload, no writeback, and one
8-byte committed store sideband. Evidence:
`python3 tools/chisel/frontend_fetch_rf_alu_qemu_rows.py --self-test`,
`bash tools/chisel/run_chisel_tests.sh --only ReducedScalarAluExecute`,
`bash tools/chisel/run_chisel_tests.sh --only LinxCoreFrontendFetchRfAluTraceTop`,
<!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `bash tools/chisel/run_chisel_frontend_fetch_rf_alu_qemu_elf_xcheck.sh --build-dir generated/r125-coremark-subi-cand-add-csdi-768-qemu-elf-xcheck --elf tests/benchmarks/build/coremark_real.elf --expected-rows 0 --capture-rows 768 --allow-block-markers --max-seconds 8 -- -nographic -monitor none -machine virt -m 1280M -kernel tests/benchmarks/build/coremark_real.elf` Provenance: source commit a4881845b415bbc01ee9a1b9899399e8ec4b76da. <!-- task15-historical-specialized-evidence:end -->,
and
...
R125 closeout: `skill-evolve: update linx-core (SUBI, C.AND trace-gap
...
| 3 | R78 trace replay xcheck harness | `tools/chisel/reduced_rob_trace_tb.cpp`, <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/run_chisel_trace_replay_xcheck.sh` Provenance: source commit ed70d74d700d5c4bcd20dc3ce768b23329d8deea. <!-- task15-historical-specialized-evidence:end -->, wrapper docs | <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `run_chisel_trace_replay_xcheck.sh` Provenance: source commit ed70d74d700d5c4bcd20dc3ce768b23329d8deea. <!-- task15-historical-specialized-evidence:end -->, top xcheck, trace self-test, QEMU dry-run |
...
| 16 | R90 QEMU trace replay harness | <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/run_chisel_qemu_trace_replay_xcheck.sh` Provenance: source commit 1c40d4eeae136370003d1e257669972e2f90e433. <!-- task15-historical-specialized-evidence:end -->, trace replay wrapper docs | dry-run, archived/fresh QEMU JSONL replay with metadata-aware raw prefix, manifest inspection |
| 17 | R91 bounded CoreMark ELF replay prefix | <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/run_chisel_qemu_trace_replay_xcheck.sh` Provenance: source commit 9cd2e09af329ff08d0370c9bb334ae72298b1281. <!-- task15-historical-specialized-evidence:end -->, trace replay wrapper docs | default-memory fail-fast check, CoreMark `--elf` replay with explicit `-m 1280M`, manifest inspection |
...
| 21 | R95 live frontend fetch RF-backed ALU trace top | `top/LinxCoreFrontendFetchRfAluTraceTop.scala`, `tools/chisel/frontend_fetch_rf_alu_trace_top_tb.cpp`, top/cross-check docs | `FrontendFetchPacketSource`, `LinxCoreFrontendFetchRfAluTraceTop`, <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `run_chisel_frontend_fetch_rf_alu_trace_top_xcheck.sh` Provenance: source commit 1775df73eefaa1eadac5a0ff0499ad133466113b. <!-- task15-historical-specialized-evidence:end -->, manifest inspection |
...
| 23 | R97 sparse ELF fetch-memory extractor | `tools/chisel/frontend_fetch_elf_memory.py`, `tools/chisel/frontend_fetch_rf_alu_trace_top_tb.cpp`, <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/run_chisel_frontend_fetch_rf_alu_trace_top_xcheck.sh` Provenance: source commit 0f74eb380193f9a56132905fc782d533e457c1b0. <!-- task15-historical-specialized-evidence:end -->, top/cross-check docs | ELF extractor self-test, live fetch RF/ALU xcheck through sparse memory, manifest inspection |
...
| 26 | R100 live QEMU capture plus reduced-row selection | <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/run_chisel_frontend_fetch_rf_alu_qemu_elf_xcheck.sh` Provenance: source commit 77001f0ec3b463950c1a275fcb5e3dc1638d73cd. <!-- task15-historical-specialized-evidence:end -->, <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `tools/chisel/build_frontend_fetch_rf_alu_qemu_fixture_elf.sh` Provenance: source commit 77001f0ec3b463950c1a275fcb5e3dc1638d73cd. <!-- task15-historical-specialized-evidence:end -->, live fetch RF/ALU harness/docs | bounded QEMU trace capture, `FETCH_QEMU_TRACE` plus `FETCH_ELF`, row-derived RF preload regression, manifest inspection, explicit unsupported-row report |
...
| 31 | R105 longer live-QEMU reduced scalar body | <!-- task15-historical-specialized-evidence:start -->historical evidence only (no current runnable equivalent): `build_frontend_fetch_rf_alu_qemu_fixture_elf.sh` Provenance: source commit 96bf4fc077ec9df6759a723c3caf5ffc13154d98. <!-- task15-historical-specialized-evidence:end -->, `LinxCoreFrontendFetchRfAluTraceTop.md`, runbook docs | `--long-body` fixture build, live QEMU marker gate with `--capture-rows 9`, seven scalar commits compared, manifest inspection |
...
| 47 | R121 CoreMark LDI/C.SETC_EQ plus bounded-capture tail prefix | `ReducedScalarAluExecute.scala`, `frontend_fetch_rf_alu_qemu_rows.py`, `frontend_fetch_rf_alu_trace_top_tb.cpp`, module/top docs | extractor self-test, execute gate, CoreMark live-QEMU gate with `--capture-rows 256`, one-hundred-seventy-three scalar/macro commits compared, tail-prefix manifest inspection |
| 48 | R122 CoreMark read-only load lookup, SETC_LTU, and local-alias row reducer | `ReducedScalarAluExecute.scala`, `LinxCoreFrontendFetchRfAluTraceTop.scala`, `frontend_fetch_rf_alu_qemu_rows.py`, `frontend_fetch_rf_alu_trace_top_tb.cpp`, module/top docs | extractor self-test, execute/top gates, CoreMark live-QEMU gate with `--capture-rows 470`, 315 normalized rows compared, 486-row redirect-marker allocation mismatch probe, 512-row `OP_SD` frontier probe, manifest inspection |
