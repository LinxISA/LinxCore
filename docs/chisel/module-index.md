...
| `DecodeRenameROBPath` | `linxcore.backend` | `docs/chisel/modules/backend/DecodeRenameROBPath.md` | Unit-green R70 reduced frontend decode to queued scalar plus T/U rename, ROB/BROB allocation, queue-backed STQ path, live ROB deallocation T/U retire-command path, scalar block-clean scheduling, and STID-carried consumed local block-commit event |
