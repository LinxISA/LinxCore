...
| `DecodeRenameROBPath` | `linxcore.backend` | `docs/chisel/modules/backend/DecodeRenameROBPath.md` | Unit-green R68 reduced frontend decode to queued scalar plus T/U rename, ROB/BROB allocation, queue-backed STQ path, live ROB deallocation T/U retire-command path, scalar block-clean scheduling, and local block-commit event observability |
...
| `ScalarTURenameBridge` | `linxcore.rename` | `docs/chisel/modules/rename/ScalarTURenameBridge.md` | Unit-green R64 scalar-GPR plus T/U rename composition owner with atomic accept, T/U operand overlay, live sequence sidecars, and retire-command acceptance feedback |
