...
| `DecodeRenameROBPath` | `linxcore.backend` | `docs/chisel/modules/backend/DecodeRenameROBPath.md` | Unit-green R72 reduced frontend decode to queued scalar plus T/U rename, ROB/BROB allocation, queue-backed STQ path, live ROB deallocation T/U retire-command path, scalar block-clean scheduling, and bridge-owned selected-STID local block-commit fanout |
...
| `ScalarTURenameBridge` | `linxcore.rename` | `docs/chisel/modules/rename/ScalarTURenameBridge.md` | Unit-green R72 scalar-GPR plus T/U bank-array composition owner with atomic accept, T/U operand overlay, live sequence sidecars, retire-command acceptance feedback, and selected-STID local block-commit fanout |
...
| `TULinkLocalBankArray` | `linxcore.rename` | `docs/chisel/modules/rename/TULinkLocalBankArray.md` | Unit-green R72 explicit `[scalar PE][STID]` T/U local-register bank-group array with bridge-owned fanout and reduced PE0/STID0 active-bank routing |
