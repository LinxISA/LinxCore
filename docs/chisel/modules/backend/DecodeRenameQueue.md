...
assign LSIDs, split stores, or rename operands. Those actions remain in their
stage owners.
...
The current `DecodeRenameROBPath` stores raw decoded rows in this queue and
stamps reduced ROB identity when the head is presented to rename. Full
model-like `DCTop::Work()` ROB reservation before queue enqueue is deferred
until the allocator exposes a true enqueue-time reservation owner.

## Deferred Owners

- BID-scoped per-row queue pruning equivalent to `SimQueue::FlushIf`.
- Width-wide decode enqueue and rename dequeue.
- Enqueue-time ROB reservation and LSID/SID assignment.
