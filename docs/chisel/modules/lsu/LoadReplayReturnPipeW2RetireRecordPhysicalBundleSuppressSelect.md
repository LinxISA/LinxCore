...
bash tools/chisel/run_chisel_frontend_fetch_rf_alu_qemu_elf_xcheck.sh \
  ...
  --build-dir generated/r608-replay-physical-suppress-promoted-select-xcheck \
...
The nonzero probe-disabled blocker and zero probe-selected counter prove the
registered boundary was fed by the promoted plan path, not by the diagnostic
probe. The five boundary captures then propagate through the existing carried
ownership, live mask, and clear-proof evidence with zero QEMU/DUT mismatches.

## Next Owner

R608 promotes only the replay fixture path under explicit default-off knobs. The
next packet should either expose selector-origin sidebands for long-window
auditing, or run a broader CoreMark no-regression window with promotion enabled
while continuing to treat the current CoreMark prefix as zero-natural-replay
coverage.
