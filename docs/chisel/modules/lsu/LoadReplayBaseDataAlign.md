...
This module does not arbitrate the top-level sparse-memory lookup port and does
not decide whether replay may launch. It only shapes data once the parent has a
selected row and a scalar load response. R295 wires it into the replay-LIQ top
while keeping `launchEnable` low and `e2ScbReturned/e2ReturnReady` false.
...
R295 does not use this as replacement evidence for live replay launch. The top
still keeps replay launch disabled and does not mux the shared sparse-memory
lookup output away from execute. Future launch enablement must first arbitrate
the memory lookup request and set SCB/return readiness from real source-return
ownership.
