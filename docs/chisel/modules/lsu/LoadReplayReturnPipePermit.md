...
R302 wires the module into the replay-LIQ top, but the top still drives
`pipeBudgetAvailable` low. The packet therefore replaces the hardcoded empty
mask with a documented owner while preserving the disabled live-relaunch
boundary.
...
| `pipeBudgetAvailable` | Future IEX return-pipe budget owner says pipe zero may be used. Current reduced top drives this low. |
...
3. require the future IEX pipe-budget owner to assert `pipeBudgetAvailable`,
...
- Real IEX return-pipe budget producer.
