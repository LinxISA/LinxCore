...
| `e2LoadDataReturned` / `e2ScbReturned` / `e2ReturnReady` | Source-return and return-slot readiness sidebands consumed by `LoadForwardPipeline`. R299 drives `e2ScbReturned` from `LoadReplaySourceReturnReadiness`. R301 drives `e2ReturnReady` from `LoadReplayReturnReadiness` fed by `LoadReplayReturnPipeSelect`; the reduced top still drives that selector's pipe mask empty. |
...
| `launchDriveValid` | Actual valid presented to `LoadInflightQueue.launchValid`; low unless the parent-owned `launchEnable && launchValid` predicate is true. Since R301, the reduced top drives return readiness from `LoadReplayReturnReadiness` fed by `LoadReplayReturnPipeSelect`; the selector's pipe mask remains empty, so launch stays disabled. |
