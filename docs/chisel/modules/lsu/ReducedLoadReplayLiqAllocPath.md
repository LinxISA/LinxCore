...
| `e2LoadDataReturned` / `e2ScbReturned` / `e2ReturnReady` | Source-return and return-slot readiness sidebands consumed by `LoadForwardPipeline`. R299 drives `e2ScbReturned` from `LoadReplaySourceReturnReadiness`. R302 drives `e2ReturnReady` from `LoadReplayReturnReadiness` fed by `LoadReplayReturnPipeSelect`; the reduced top still drives `LoadReplayReturnPipePermit.pipeBudgetAvailable` low. |
...
| `launchDriveValid` | Actual valid presented to `LoadInflightQueue.launchValid`; low unless the parent-owned `launchEnable && launchValid` predicate is true. Since R302, the reduced top drives return readiness from `LoadReplayReturnReadiness` fed by the permit/select split; the permit budget remains low, so launch stays disabled. |
