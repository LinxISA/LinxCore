...
for the three W2 side-effect sinks. LinxCoreModel `LDAPipe::runW2` and
...
R357 replaces the previous three top-local `false.B` ties with one module that
computes the required sink mask and the live-enable mask. The reduced top
still ties `liveRequested` false, so resolve, writeback, and wakeup sinks stay
not-ready and no architectural state changes.
...
| output | `resolveLiveEnable` | Live enable for `LoadReplayReturnPipeW2ResolveSinkReady`. |
| output | `writebackLiveEnable` | Live enable for `LoadReplayReturnPipeW2WritebackSinkReady`. |
| output | `wakeupLiveEnable` | Live enable for `LoadReplayReturnPipeW2WakeupSinkReady`. |
...
- `resolveLiveEnable`, `writebackLiveEnable`, and `wakeupLiveEnable` drive
  the R336/R337/R338 sink owners instead of local constants;
...
completion, R334 does not clear the W2 slot, and R351/R356 promotion remains
dormant.
