...
`headerPc`/`hsize`/`bsize` geometry when a later block boundary or `BSTOP`
closes the current body.
...
event in the current Chisel metadata.
...
| output | `headerActive`, `learnedFire` | `Bool` | Reduced diagnostics for active header state and learn events. |
...
the combinational geometry for the cycle, while `f4UpdateFire` gates the
registered active-header update:
...
yet a producer event, and elaborate the Chisel module.
