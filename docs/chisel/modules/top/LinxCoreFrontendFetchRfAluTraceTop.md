...
- Child owners:
  ...
  - `rtl/LinxCore/chisel/src/main/scala/linxcore/lsu/LoadReplayReturnSideEffectLiveControl.scala`
...
R467 revalidates the smaller 6000-row admitted-marker CoreMark gate after the
R466 live-top maintenance packet split dense MDB visibility into a helper and
added the unconnected wait-plan bridge sidecar. The post-maintenance run
captures 6000 raw QEMU rows, extracts 5866 expected rows, admits and filters
728 marker commits, normalizes 5138 QEMU/DUT rows, compares 5137 rows, and
passes with zero mismatches and no CBSTOP divergence. The sideband stats keep
the default marker-row top's replay-LIQ counters at zero, confirming the
maintenance packet did not accidentally enable the reduced replay-LIQ path.
...
R492 extends the same admitted-marker CoreMark path to a 393216-row capture.
The run extracts 393082 expected rows, admits and filters 28387 marker commits,
normalizes 364695 QEMU/DUT rows, compares 364694 rows, and passes with zero
mismatches and no CBSTOP divergence. BFU diagnostics remain aligned across the
larger window: 83889 static/external matches, 83888 accepted body-cut arms with
zero arm mismatches, and 27961 promoted runtime body-end oracle replay matches.
The default marker-row top replay-LIQ counters remain zero over 2614953 sampled
cycles. This supersedes R468 as the largest admitted-marker reduced
fetch/RF/ALU CoreMark replacement proof while full LSU/full-DUT CoreMark
...
R473 extends the wrapper-validated sideband stats schema to v2 with a
model-derived wait-stimulus classifier. LinxCoreModel `STQ::lookupForLoad`
marks `wait_store` only when a younger load overlaps an older address-ready
resident STQ row whose data bytes are still not ready; `LDQInfo::waitStore`
then holds the load until the store unit wakeup clears that key. The v2 report
therefore records execute load-lookup activity, resident eligible-store
cycles, lookup/store overlap cycles, ready-forward/wait-blocked predicates,
STA/STD queue and execution validity, plus the existing replay-LIQ/MDB
counters. This does not add RTL ports or change compare behavior; it makes
future live probes distinguish no load lookup, no resident store, no overlap,
and wait-predicate failure before claiming a nonzero replay-LIQ/MDB stimulus.
The R473 fixture run passes 3 compared rows with zero mismatches and classifies
the current negative probe as load/STQ timing rather than LIQ mutation:
`load_lookup_execute_granted=2`, `store_sta_exec_valid=1`,
`store_std_exec_valid=2`, but `resident_store_eligible=0`,
...
R388 extends the same replay-LIQ namespace with
`LoadReplayReturnIexPipeOccupancyLiveControl`. The reduced top now feeds the
IEX occupancy owner's `liveRequested` and `livePipeOccupiedMask` from a named
...
R394 adds `LoadReplaySourceReturnStoreSnapshotIdentityMatch` as the `cID/eID`
and stale-row identity owner for that response-match input. The first direct
dormant top integration attempt hit the top constructor `Method too large`
limit. R395 relieves that pressure by replacing the separate R390-R393 top
children with one `LoadReplaySourceReturnStoreSnapshotPath` child. The path
contains query issue, identity match, response match, evidence, and
ready-control; the top helper ties the path's live request, sink, selected-row
identity, raw response, SCB, wait-store, and data inputs false while preserving
all existing top diagnostics. R396 adds
`LoadReplaySourceReturnStoreSnapshotSelectedIdentity` inside that composite:
the top passes the existing reduced LIQ launch index and `repickMask` to the
path, which projects a single-cluster selected-row identity while raw
request/response inputs remain disabled.
R397 adds `LoadReplaySourceReturnStoreSnapshotAcceptedToken` inside the same
composite. Query issue is now gated by one-token capacity inside the path, and
...
live replay launch.
R399 adds `LoadReplaySourceReturnStoreSnapshotResponseDrain` inside the
composite. The raw FIFO now pops only when the response is ordered or when a
future row-state owner proves the head stale. The top ties stale-head evidence
false, so no stale drop is live yet.
R400 adds `LoadReplaySourceReturnStoreSnapshotResponseHeadState` inside the
...
R402 adds `LoadReplaySourceReturnStoreSnapshotRequestPayload` inside the
composite. The helper forwards existing selected-row PC/address/size/BID/LSID
diagnostics into the path so the future local STQ lookup queue has a typed
payload, but live request enable remains tied false.
R403 adds `LoadReplaySourceReturnStoreSnapshotRequestQueue` inside the
composite. Query issue is now gated by internal queue capacity, and the helper's
existing `sinkReady=false` tie-off means the queued request head cannot drain to
the still-deferred raw store-unit sink.
...
the R356 promotion request from one top-level request gate. R534 enables that
top-level gate with snapshot-fed prerequisites, so later positive replay-return
gates must inspect the atomic request, side-effect, row-fill, and lifecycle
counters rather than assuming the default fixture exercises W2 mutation.
...
| output | `reducedMdbConflict*` | mixed | diagnostic | R659 canonical `ScalarLSUMDBPath` conflict visibility. Accepted reduced STQ probes scan resident LIQ and ResolveQ rows; record, retained multi-row wait plan, and typed Linx recovery enqueue share one transaction. The report enters `ScalarLSURecoveryBoundary`, owner-backed STID-local age, exact full-BID lookup, and central recovery acceptance. |
| output | `reducedMdbFanout*` | mixed | diagnostic | R659 canonical fanout/learning visibility. LIQ allocation and PC-keyed lookup accept atomically from one payload. Failed-wait delete/decay is live, and registered MDB store wakeup enters the LIQ wake port ahead of the held resident-store wake source. Lookup/delete/record, BMDB, SSIT, and wakeup outputs remain observable without a duplicate fanout owner. |
| output | `reducedMdbLookupWaitPlan*` | mixed | diagnostic | R659 canonical lookup wait-plan visibility. The planner is a child of `ScalarLSUMDBPath`; its request shares `LoadReplayRowMutationSourceMux` with source-return mutation. MDB-only requests may mutate pre-launch `Wait` rows without SCB-return proof, while source-return requests keep their stricter ordering. The nested bridge remains shape diagnostics for the canonical request. |
...
- `LoadReplaySourceReturnStoreSnapshotPath`: optional R395 composite local STQ
  snapshot source-return path. It contains the R401 request control, R402
  request payload, R403 request queue, R392 query issue, R396 selected-identity
  projection, R397 accepted-query token, R398 response queue, R400
  response-head state proof, R399 response drain, R394 identity match, R393
  response match, R391 evidence, and R390 ready-control owners under one top
  child instance.
...
- `LoadReplaySourceReturnStoreSnapshotRequestPayload`: optional R402
  selected-row request shaper inside `LoadReplaySourceReturnStoreSnapshotPath`.
  It copies the accepted repick row's local identity, BID/GID/RID, load LSID,
  PC, address, size, and byte mask into a typed payload for the future local
  STQ lookup queue.
- `LoadReplaySourceReturnStoreSnapshotRequestQueue`: optional R403 request FIFO
  inside `LoadReplaySourceReturnStoreSnapshotPath`. It stores R402 selected-row
  payloads before the future raw store-unit sink and keeps queue-head
  visibility independent of sink readiness.
...
- `LoadReplaySourceReturnStoreSnapshotEvidence`: optional R391 selected-row
  ...
  reaches this owner through the R395 path while keeping request issue and
  response evidence disabled and preserving legacy combinational snapshot-ready
  behavior.
...
- `LoadReplaySourceReturnStoreSnapshotSelectedIdentity`: optional R396 reduced
  selected-row identity projection inside
  `LoadReplaySourceReturnStoreSnapshotPath`. The current reduced top feeds it
  from `ReducedLoadReplayLiqAllocPath.launchIndex` and `repickMask`, but keeps
  raw STQ responses disabled.
- `LoadReplaySourceReturnStoreSnapshotAcceptedToken`: optional R397
  accepted-query token owner inside `LoadReplaySourceReturnStoreSnapshotPath`.
...
- `LoadReplaySourceReturnStoreSnapshotResponseHeadState`: optional R400
  reduced row-state proof inside
  `LoadReplaySourceReturnStoreSnapshotPath`. It classifies only cluster-0,
  in-range heads against the current `repickMask` and leaves unsupported
  identities held.
- `LoadReplaySourceReturnStoreSnapshotResponseDrain`: optional R399 raw STQ
  response head consume/drop owner inside
  `LoadReplaySourceReturnStoreSnapshotPath`. The current reduced top ties
  stale-head evidence false, so the owner only reserves the future model-stale
  drop boundary.
- `LoadReplaySourceReturnStoreSnapshotResponseMatch`: optional R393 selected-row
  ...
  reaches this owner through the R395 path and ties raw response, selected-row
  identity, and SCB-return evidence low at the path boundary.
...
- `LoadReplayReturnWritebackSinkReady`: optional R380 pre-W2 replay RF
  writeback sink readiness owner. It observes the writeback candidate plus the
  execute-priority scalar RF port and keeps the sink not-ready while live replay
  RF mutation is disabled.
- `LoadReplayReturnWakeupSinkReady`: optional R379 pre-W2 wakeup sink
  readiness owner. It observes the wakeup candidate, arms abstract
  ready-table/issue-wakeup capacity, and keeps the sink not-ready while live
  mutation is disabled.
- `LoadReplayReturnSideEffectLiveControl`: optional R381 pre-W2 side-effect
  live-control owner. It computes publish/writeback/wakeup required and
  live-enable masks, then feeds the publish-control, writeback-sink, and
  wakeup-sink live-enable inputs while its live request remains disabled.
...
- `LoadReplayReturnLretSink`: optional R318 diagnostic LRET FIFO owner. It
  ...
  output while R383 `LoadReplayReturnIexPipeOccupancy` keeps the permit false.
- `LoadReplayReturnIexPipeOccupancy`: optional R383 diagnostic IEX return-pipe
  occupancy owner. It replaces the top-local tied-full mask with a
  live-disabled forced-full owner feeding the drain permit until real return
  pipe state is wired.
- `LoadReplayReturnIexPipeOccupancyLiveControl`: optional R388 live request
  owner for the IEX return-pipe occupancy source. It replaces the top-local
  `liveRequested=false` and zero-mask constants while the reduced top still
  keeps the request and source disabled.
- `LoadReplayReturnIexDrainPermit`: optional R319 diagnostic IEX-side drain
  ...
  FIFO drain input, and R383 feeds its occupancy input from the forced-full
  owner, so no drain fires.
...
- `ScalarLSUMDBPath`: R659 canonical replay-LIQ MDB owner. It contains the sole
  live `MDBConflictDetect`, `MDBQueueFanout`, lookup wait planner, failed-wait
  timeout/delete path, and retained recovery queue. Accepted allocation payload
  drives lookup before first launch; store probes, LIQ rows, and ResolveQ rows
  drive conflict publication. Its registered store wakeup is consumed by LIQ,
  and its retained report enters the shared Linx recovery boundary.
- `LoadReplayRowMutationSourceMux`: joins source-return and canonical MDB row
  writes. Same-target requests coalesce. Different-target requests preserve
  source-return priority and retain the MDB command until the native LIQ write
  accepts. Per-source control selects the strict source-return or canonical
  MDB pre-launch mutation policy.
...
selected-row match, and SCB-return evidence disabled. R394 adds the identity
owner for that selected-row match input. R395 replaces the separate
store-snapshot top children with `LoadReplaySourceReturnStoreSnapshotPath`, so
the identity owner is integrated inside the composite path without new top IO
or another direct top child instance. R396 feeds that path with the reduced
LIQ launch index and repick mask through
`LoadReplaySourceReturnStoreSnapshotSelectedIdentity`, preserving the existing
...
the top still ties raw response inputs false. R399 inserts
`LoadReplaySourceReturnStoreSnapshotResponseDrain` after response matching so
the FIFO pop decision is separated from token clear, while stale-head evidence
...
future live request promotion. R402 inserts
`LoadReplaySourceReturnStoreSnapshotRequestPayload` after query issue so the
future `lookup_lu_su_q` request path has a typed selected-row payload. R403
inserts `LoadReplaySourceReturnStoreSnapshotRequestQueue` behind that payload;
the queue is internally present but cannot drain while the top ties the future
raw sink readiness false. R300 inserts
...
`enqueueReady` capacity signal. R379 replaces the raw wakeup-sink false tie-off
with a live-disabled readiness owner that can arm abstract ready-table and
issue-wakeup capacity. R380 replaces the pre-W2 writeback readiness source
with `LoadReplayReturnWritebackSinkReady`, a live-disabled owner that observes
execute-priority scalar RF write-port capacity. R381 replaces the local false
live-enable tie-offs for publish, writeback, and wakeup with
`LoadReplayReturnSideEffectLiveControl`; its `liveRequested` input remains
false. The side-effect predicate
therefore reports real LRET capacity plus named writeback/wakeup blockers but
remains unable to enable live replay state mutation.
...
permit still remains false because R383 forces the current pipe-occupied mask
full through `LoadReplayReturnIexPipeOccupancy`.
...
pipe is free. R383 inserts `LoadReplayReturnIexPipeOccupancy` before this
permit, and R388 inserts `LoadReplayReturnIexPipeOccupancyLiveControl` before
the occupancy owner; the reduced top keeps the live request and source disabled,
so the selected occupancy mask is still full. The top drives
`LoadReplayReturnLretSink.drainReady` from this permit, exposing the future
pipe-full blocker without draining or discarding returned payloads.
...
R384 keeps `liveEnable` disabled through `LoadReplayReturnPipeResidencyLiveControl`,
so it reports LDA target intent plus occupied-pipe and live-disabled blockers
without mutating E4 pipe residency.
...
still disables that write path through R384, so the slot reports dormant
...
R550 keeps the live-gated side-effect readiness join intact, but feeds the W2
atomic prerequisite snapshot from pre-request sink capacity: resolve
`resolveArmed`, optional writeback `writebackArmed`, and optional wakeup
`wakeupArmed`. Actual resolve/writeback/wakeup mutation is still gated by
`LoadReplayReturnPipeW2SideEffectLiveControl`, so the packet breaks the policy
cycle without firing W2 side effects early. The replay-loop fixture passes with
9 compared rows, zero mismatches, and zero QEMU/DUT CBSTOP rows. The v21
sideband report moves the ordered blocker to clear-commit readiness:
`w2_atomic_blocked_by_no_side_effect_sink=7`,
`w2_atomic_blocked_by_no_clear_commit=67`,
`w2_clear_intent=0`, `w2_clear_commit_ready=0`, and
...
diagnostics. R465 adds `LoadReplayMdbLookupWaitPlan` as a diagnostic consumer
