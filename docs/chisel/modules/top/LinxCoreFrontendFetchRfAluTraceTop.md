...
| output | `reducedLoadReplayResolveQueue*` | mixed | diagnostic | R285-R287 opt-in replay-LIQ ResolveQ diagnostics. `lhqRecord` from `ReducedLoadReplayLiqAllocPath` can append to `LoadResolveQueue`; the top exposes push, delayed clear, commit-window retire watermark, retire mask/count, occupancy, valid-mask, and head conflict-row sidecars. Because launch remains disabled, the current fixture observes storage and retire wiring only; live MDB publication is deferred. |
...
- `LoadResolveQueue`: optional R285 replay-LIQ ResolveQ storage boundary. It
  ...
  ResolveQ retire watermark. Precise flush pruning and live MDB publication
  remain separate owner packets. R286 also makes the top own a one-entry delayed
