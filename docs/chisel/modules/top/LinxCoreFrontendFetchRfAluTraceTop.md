...
| output | `reducedLoadReplayLiqReplayWake*` | mixed | diagnostic | R509 replay-LIQ wake-match diagnostics. These expose whether the forwarded/held resident-store replay wake is valid, overlaps wait-store rows, matches BID, optional LSID, PC, and `StoreUnit` source, and whether the child LIQ clear mask fires. |
