...
sink is ready, but `liveEnable=false`, so W2 resolve still blocks completion
without mutating ROB state.
...
`liveEnable=false`, so W2 writeback still blocks completion without mutating RF
state.
...
ready-table/issue-wakeup sink is ready, but `liveEnable=false`, so W2 wakeup
still blocks completion without mutating ready-table or issue-queue state.
