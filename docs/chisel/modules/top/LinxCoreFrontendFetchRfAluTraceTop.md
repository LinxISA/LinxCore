...
| output | `loadLookupValid`, `loadLookupAddr`, `loadLookupPc`, `loadLookupExecuteGranted`, `loadLookupReplayGranted` | mixed | diagnostic | Arbiter-selected reduced sparse-memory load lookup request. Execute has priority for `OP_C_LDI`/`OP_LDI` and resident wait-store replay-slot loads; replay-LIQ base lookup may drive the port only when execute is idle. The replay path also carries the row's BID/GID/RID internally. |
...
| output | `reducedLoadWaitReplay*` | mixed | diagnostic | R269/R271/R274/R275/R307 registered reduced wait-store slot diagnostics. The slot remembers the held load and wait-store key, feeds that key back to `ResidentStoreReplayWakeup`, clears through `LoadReplayWakeup`, and publishes a one-cycle relaunch candidate carrying PC, address, size, replay-return signedness, BID/GID/RID, reduced LSID, and forwarding snapshot `(youngestStoreId, youngestStoreLsId)` for future LIQ wiring; it does not drive a launch port. |
| output | `reducedLoadReplayQueue*` | mixed | diagnostic | R272-R283/R307 finite relaunch-candidate queue diagnostics. The queue consumes the one-cycle wait-slot candidate, records pending/outgoing full load identity plus replay-return signedness and forwarding snapshot, and reports accepted/drop/full state. Default mode drains only exact held-load completion matches; opt-in replay-LIQ mode consumes only when `ReducedLoadReplayLiqAllocPath` accepts allocation. |
...
- `LoadReplayReturnLretPayload`: optional R310 diagnostic LRET/MemReqBus
  ...
  data, pipe index, and wakeup-required sideband once extractor data is valid.
...
sideband once `LoadReplayReturnDataExtract.dataValid` is true. It deliberately
does not invent destination physical tags, enqueue LRET, wake dependents, or
feed replay launch readiness.
