...
- `LoadReplayReturnPipeBudget`: optional R303 live-budget-arm owner for
  replay-LIQ return pipes. Current top wiring still ties its budget arm low.
...
producer. R303 inserts `LoadReplayReturnPipeBudget` as the explicit live-budget
owner feeding the permit while keeping the future budget arm low. `launchEnable`
therefore remains low while source-return, budget, and return-port blockers are
