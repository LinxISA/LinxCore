...
| output | `reducedLoadReplayLiq*` | mixed | diagnostic | R279/R281/R282/R283/R294 opt-in replay-LIQ allocation and launch-selector diagnostics. When the replay-LIQ wrapper is selected, the queue head feeds `ReducedLoadReplayLiqAllocPath`, queue dequeue is driven only by LIQ allocation acceptance, and `reducedLoadReplayLiqLaunch*` reports which resident rows would satisfy the model `pickL1` data-hit predicate. R282 adds a path-local launch drive gate, R283 feeds its E2 store vector from resident STQ snapshots, and R294 exposes selected launch-row PC/address/size, load ID, BID/GID/RID, load LSID, and request-byte mask; this top still ties launch disabled. |
