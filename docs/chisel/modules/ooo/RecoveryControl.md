...
`RecoveryControl` closes recovery R4 across the external CTU, exact
...
`CTU`. `ctuPrepared` must echo that exact request and may report
...
`fence` connects to `OOOD1D2Stage`, `OooD2Stage`, and every
