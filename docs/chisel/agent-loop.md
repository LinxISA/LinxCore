...
R169 narrows the next block-control packet: consume
`robDeallocBlockMarkerRetireSource` through a serializer or lifecycle adapter,
feed `BlockMarkerLifecycle` from retired marker rows, add per-STID active marker
state, and prove recovery-exact cleanup before claiming full marker-row block
execution.
...
1. Full marker ROB retirement and recovery: R168 moved reduced active-BID
   lifecycle into `BlockMarkerLifecycle`, but marker rows are still consumed
   outside ROB. The next block-control packet must feed this BCTRL owner from
   full marker-row retirement, add per-STID active block state, and prove
   recovery-exact marker cleanup before claiming full block execution.
