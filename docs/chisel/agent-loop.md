...
R172 narrows the next block-control packet: add per-STID active marker state,
prove recovery-exact marker-source pruning, and replace the reduced marker-skip
path with full marker-row ROB admission before claiming full marker-row block
execution.
...
1. Full marker ROB admission and recovery: R172 feeds serialized retired marker
   rows into `BlockMarkerLifecycle` with row-owned BID evidence, but the
   reduced marker-skip lane still bypasses ROB admission. The next
   block-control packet must add per-STID active block state, prove
   recovery-exact marker-source cleanup, and replace marker-skip with full
