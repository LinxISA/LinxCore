...
2. Promote beyond the R148 BFU agreement diagnostic boundary. The reduced top
   can compute cut/restart PCs from `headerPc`/`hsize`/`bsize`, and a
   diagnostic producer now learns `bsize` from explicit block-boundary,
   `BSTOP`, or resolved body-end events while carrying resolved `hsize`
   payload and proving agreement with the external replay geometry. The next
   packet should add a real branch-resolution/body-end owner before driving
   `ReducedBfuBodyCutPredictor` from decoded frontend state.
