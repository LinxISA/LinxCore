...
1. Wire live block/group commit clean events into
   `TULinkRetireCommandPath.cleanBlock*` and `cleanGroup*` once the block
   commit owner publishes model `CleanCMAP` and `CleanGroupCMAP` events.
