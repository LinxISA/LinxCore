...
1. Scale the admitted-marker CoreMark gate beyond the R202 1024-row window.
   ...
   restoring marker-stop cleanup from the post-block checkpoint BID. The
   promoted 1024-row run admits and filters 288 marker commits, compares 665
   normalized rows, and passes with zero mismatches. The next packet should
   grow the filtered marker-row window while watching block-stop cleanup BID
   selection, post-rename checkpoint refresh, marker-only BROB retire drain,
   and loop re-entry. Reproduce the closed baseline with
   `generated/r202-marker-stop-restore-qemu-elf-xcheck`.
