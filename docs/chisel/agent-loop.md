...
1. Debug the R198 post-capacity source-value mismatch in the admitted-marker
   1024-row CoreMark gate. R195 proved scalar GGPR mapQ capacity was the
   blocker at `pc=0x4000557a` with `decodeBlockedByRename=1`, `gprFree=63`,
   and `gprMapQFree=0`; R196 split scalar `gprMapQDepth = 256` from the compact
   local T/U `mapQDepth = 32`, matching LinxCoreModel `ggpr_mapq_depth = 256`;
   and R198 made the 256-depth checkpoint path Verilator-practical with
   `GPRRenameReplaySurvivorSelect` and `GPRRenameCommitArchSelect`. The same
   1024-row gate now builds and fails functionally at `pc=0x400055be`,
   `insn=0x31f6`: QEMU expects source `x6 = 296`, while the DUT observes
   `x6 = 0` with `src_phys=(46,20,0)` and last writer for source-1 physical tag
   20 at `pc=0x40005576`, `insn=0xffe13319`, `writer_wb=6`,
   `writer_data=0`. The next packet should inspect reduced RF writeback/read,
   source readiness, and any marker-row path source forwarding before changing
   scalar mapQ capacity again. Reproduce with
   `generated/r198-helper-split-marker-row-1024-qemu-elf-xcheck`.
