...
1. Scale the admitted-marker CoreMark gate beyond the R228 8192-row window.
   ...
   restoring marker-stop cleanup from the post-block checkpoint BID. R227 and
   R228 extend the same admitted-marker path on clean pushed LinxCore/Model/QEMU
   SHAs to 6000 and 8192 raw QEMU rows; the latest run extracts 8058 expected
   rows, filters 885 marker commits, compares 7172 normalized rows, and passes
   with zero mismatches. The next packet should grow beyond the 8192-row window
   while watching block-stop cleanup BID
   ...
   `generated/r227-row-order-6000-marker-qemu-elf-xcheck` or the latest
