...
R170 narrows the next block-control packet: integrate
`BlockMarkerRetireSourceSerializer` into the backend deallocation path, feed
`BlockMarkerLifecycle` from serialized retired marker rows, add per-STID active
marker state, and prove recovery-exact cleanup before claiming full marker-row
block execution.
