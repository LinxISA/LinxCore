...
2. Promote beyond the R146 BFU resolved-end diagnostic boundary. The reduced
   top can compute cut/restart PCs from `headerPc`/`hsize`/`bsize`, and a
   diagnostic producer now learns `bsize` from explicit block-boundary,
   `BSTOP`, or resolved body-end events. The next packet should add a real
   branch-resolution/body-end owner and settle the `hsize` contract before
   driving `ReducedBfuBodyCutPredictor` from decoded frontend state.
