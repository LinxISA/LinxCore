...
R173 narrows the next block-control packet: add per-STID active marker state
and replace the reduced marker-skip path with full marker-row ROB admission
before claiming full marker-row block execution.
...
1. Full marker ROB admission and per-STID state: R172 feeds serialized retired
   marker rows into `BlockMarkerLifecycle` with row-owned BID evidence, and R173
   gives the marker-source queue recovery-exact suffix pruning. The reduced
   marker-skip lane still bypasses ROB admission. The next block-control packet
   must add per-STID active block state and replace marker-skip with full
