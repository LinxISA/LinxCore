...
| I0.9d scalar-load tracking/replay | Implemented | parameterized multi-entry tracker; producer-qualified generation; fair memory request; speculative wakeup; exact hit/miss/fault response; size/sign extension; retained W1 bypass/result; miss cancel and new-generation retry; stale response rejection; grouped-ROB recovery | add physical cache/TLB adapter, atomic RF/committed-wakeup/ROB sink, store path, multi-port bandwidth, and end-to-end static-top IT |
