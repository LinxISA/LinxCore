...
| I0.9a typed ALU pipeline | Implemented | generated-recipe-aligned opcode whitelist; E1 compute; retained W1 bypass and W2 terminal owner; GPR/T/U destination preservation; exact recovery/load-cancel; malformed class/recipe/shape rejection | add D1 source-modifier controls, register ALU forms, and static-top terminal connection |
| I0.9b typed BRU pipeline | Implemented | D1-normalized 32/48-bit ADDTPC byte displacement; generated-recipe-aligned BRU/BCTRL whitelist; E1 value/control compute; retained atomic E2 transaction; exact recovery/load-cancel; malformed and redirect forms fail closed | add architectural condition input, J/JR and B.Z/B.NZ, redirect sink, and static-top terminal connection |
| I0.9c typed scalar-load AGU | Implemented | D1 typed memory mode/size/sign/byte-offset/index-transform/source masks; payload-sidecar preservation; all generated ScalarLoad recipes; E1 byte-address compute; retained LSU request; exact recovery/load-cancel | add load-generation allocator/tracker, LSU return and wakeup, replay/cancel/repick, store AGU/STD projection/join, and static-top connection |
| I0.9d scalar-load tracking/replay | Implemented | parameterized multi-entry tracker; producer-qualified generation; fair memory request; speculative wakeup; exact hit/miss/fault response; size/sign extension; retained W1 bypass/result; miss cancel and new-generation retry; stale response rejection; grouped-ROB recovery | add physical cache/TLB adapter, store path, multi-port bandwidth, and end-to-end static-top IT |
| I0.9e atomic terminal publication | Implemented | fair ALU/BRU/load normalization; exact route/member/destination validation; duplicate destination rejection; atomic multi-destination P/T/U write plus committed wakeup, optional BCTRL, terminal trace, and ROB completion; precise load fault/cause retained per ROB member and pruned by suffix recovery without RF mutation; real OperandFiles composition IT; valid-independent RF write preflight | connect grouped ROB/BCTRL/IQ/trace and all typed pipes in the canonical static top; measure multi-pipe terminal bandwidth |
| I0.9f store decode and AGU/STD projection | Implemented | generated indexed-store source counts; generator-owned child-specific PC-read metadata; scalar/pair size, offset, index, PCR, and pre/post-writeback normalization; disjoint exhaustive source masks; fixed child0=AGU/child1=STD validation; per-child IQ source projection; AGU-only writeback destination | implement retained store AGU and STD owners, exact join, canonical STQ/LSU ordering and visibility, then static-top IT |
| I0.9g canonical memory-order identity | Implemented | independent per-STID full-LSID/LID/SID tails; all-or-none D2/D3 lease; no-bubble same-STID publish/replace; per-logical-uop ranges retained in ROB and IEX; common S1 publication; private cancel rollback; exact partial-pivot/global recovery from ROB-owned snapshots; 40-bit and O3-to-IEX UT/IT | serial-wrap quiescence and ordered visibility; full serials must never be queue indices |
| I0.9h canonical STQ lease owner | Implemented | physical row reservation before execute; exact ROB/BROB owner plus full LSID/store-ID; generation-qualified lease fill; stale/wrong-owner rejection; unequal STQ/ROB and 40-bit tests | remove legacy CAM insert after static-top migration |
| I0.9i retained STA/STD and pair reservation | Implemented | logical-member normalization; scalar one-row and pair all-or-none two-row reservation; pair-credit fencing; independent retained STA/STD; mask-derived address/data; pair beat generation; canonical STQ-only convergence | replace the single-lease test harness with formal multi-store static residency |
| I0.9j common store/STQ recovery | Implemented | one ROB-authorized recovery plan drives retained STA/STD cancellation and exact STQ free projection; WAIT-only free; committed-row blocking; exact-over-legacy source arbitration; lease generation never rewinds | instantiate in the canonical static IEX/LSU top and delete compatibility recovery |
| I0.9k cross-bank store issue frontier | Implemented | compact AGU/STD schedule-row logical-store key; per-STID full-store-ID oldest reduction; both children of the oldest logical store admitted; younger same-STID stores blocked across classes; peer STIDs and non-stores bypass; exact release/recovery recomputation; wrap and duplicate-owner UT/IT | connect STQ issue-window credit, safe-mode/liveness counters, commit frontier, and static top |
| I0.9l exact store CommitQ frontier | Implemented | exact owner + physical lease generation + full LSID/store-ID token; per-STID oldest committed-store eligibility; stalled-old blocks younger same-STID while peers bypass; token-to-STQ row revalidation; WAIT-to-Commit same-edge snapshot; ordinary recovery preserves committed tokens; split last-fragment-only free ownership; dynamic UT/IT | add retained fragment owner, logical pair completion, exact ROB commit-token ingress, MMIO serializer, and static top |
| I0.9m retained committed fragment owner | Implemented | CommitQ launch decoupled from SCB ready; registered exact token/request batch; stable valid/address/data/size/identity under multi-cycle backpressure; retained row/lease revalidation; atomic batch acceptance; accepted last-fragment free alignment; dynamic backpressure UT/IT | add logical pair/four-fragment completion, MMIO serializer, exact ROB commit-token ingress, and static top |
| I0.9n logical pair-store completion | Implemented | scalar/pair group metadata retained from STQ reservation through fill and CommitQ; incomplete pair blocks same-STID younger stores while peers bypass; complete pair atomically occupies two issue lanes even when tokens are physically interleaved; two cross-line beats retain four stable fragments; accepted batch frees two physical rows and emits one exact logical completion; queue/drain/SCB UT/IT | add exact ROB commit-token ingress, MMIO serializer, serial-wrap quiescence, and static top |
| I0.10 production static execution composition | Implemented | formal 14-lane route; six ALU, two BRU, three load-address/load-tracker owners; explicit retained store/multicycle/system/PAuth/FSU/CMD boundaries; two independent cluster-local atomic terminal lanes; canonical P/T/U write, wakeup, bypass, load-cancel, BCTRL, trace, and exact ROB completion feedback; one common recovery plan; route/terminal UT, adjacent S1-to-E1 and atomic-RF IT, plus full-top structural elaboration | close monolithic IQ generated-graph cost, retained store owners and canonical STQ, physical TLB/L1D, later execution families, default-width timing, and O9 top promotion |
| I0.11a canonical multi-store STQ execution fabric | Implemented | retained S1 store-reservation barrier before S2; fair per-STID reservation of every logical store; two STA/two STD retained owners; STQ-row-only exact generation-qualified lease lookup; fair canonical fill arbitration; common WAIT-only recovery; multi-store/unreserved/recovery UT plus focused issue barrier proof | add the direct execution/store wrapper, exact ROB-to-CommitQ ingress, SCB drain integration, store-data bank, forwarding/replay, translation/L1D/coherence, timing, and O9 promotion |
| I0.11b execution/store production wrapper | Implemented | direct private reservation/STA/STD/load-cancel wiring between the formal fourteen-lane execution pipeline and canonical STQ; two-owner recovery prepare with one common fire; prepare-time STQ mutation fence; post-IQ split-store partial-cut rejection; full wrapper structural gate and dynamic execution-to-STQ IT | add exact ROB-to-CommitQ ingress, canonical SCB drain, physical store-data bank, forwarding/replay, translation/L1D/coherence, timing, and O9 promotion |
| I0.12a canonical store commit backend | Implemented | external-row backend preserves one physical STQ owner; semantic ROB token rediscovers one exact converged lease; typed PMA sidecar gates atomic WAIT-to-COMMIT plus CommitQ insertion; cacheable SCB and serialized non-cacheable/MMIO terminal owners return the only free masks; recovery fences new work while preserving committed transport; full-wrapper structural gate and real STA/STD-to-STQ-to-CommitQ-to-SCB IT | connect the O3 coordinator token source at the canonical top, physical DTLB/PMP/PMA and device transports, forwarding/replay, store-data bank, timing, O9 promotion, and workloads |
| I0.12b closed O3/IEX/store production composition | Implemented | retained all-visible completion buffer accepts two execution lanes plus fast resolve and recovery-compacts exact survivors; dedicated fifth P write/eighth wakeup ports terminate fast results in the canonical operand owner; formal capability topology; private S1/release/recycle/completion/store-token/PC/recovery seams; bounded full-hierarchy elaboration plus focused fast-result, O3/IEX, and ROB-to-STQ IT | physical multiwrite ROB option, DTLB/PMP/PMA/device wiring, forwarding/replay, store-data bank, later FU owners, default-width timing, O9 core-top promotion, and workloads |
