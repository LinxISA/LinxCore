...
| `frontend-fetch-rf-alu-trace-top-xcheck` | `tools/chisel/run_chisel_frontend_fetch_rf_alu_trace_top_xcheck.sh` | Emit `LinxCoreFrontendFetchRfAluTraceTop`, build the Verilator harness, drive PC request/response windows through `FrontendFetchPacketSource`, F4, reduced rename/ROB, RF-backed issue, and ALU execute, then compare dependent scalar commit rows against QEMU-shaped reference rows. |
...
the RF-backed reduced issue and ALU path. The fixture still serves bounded
instruction windows, but Chisel now owns packet creation, F4 byte-count PC
advance, rename/ROB allocation, RF source data, issue residency, ALU
completion, and commit-row export for the three dependent scalar rows.
